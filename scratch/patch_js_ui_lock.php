...
// 1. Fix _triggerAutoBreakStart payload
$s1 = <<< 'JS'
...
// 2. Fix polling logic when starting
$s2 = <<< 'JS'
...
JS;
$r2 = <<< 'JS'
        if (serverDown && serverDown.jenis_downtime === 'break time' && !clientBreak) {
            if (serverBreakIsAuto) {
                window._autoBreakActive = true;
                window._autoBreakDowntimeId = serverDown.id;
                const serverBreakWindow = _isInBreakWindow(new Date());
            ...
            if (serverBreakIsAuto) {
                ...
                // If manual break, ensure overlay is hidden
...
// 3. Fix polling logic when ending
$s3 = <<< 'JS'
...
// Fix mid-break detection on page load (so it checks source === 'AUTO' and calls _updateBreakUI ONLY if true)
$s4 = <<< 'JS'
                window.ProductionConfig.currentDowntimeCount = Object.keys(window.runningDowntimes).length;
...
echo "Patched production-engine.js\n";
