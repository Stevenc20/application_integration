...
def is_header_row(row, col2_val):
    """True if this row is the data header row (contains NO. at col 2 or JOB MASTER)."""
    if col2_val is not None:
        s = str(col2_val).strip().upper()
        if s in ('NO.', 'NO'):
            return True
    # Fallback: JOB MASTER is unique to data header rows, catches corrupted cells like 'MB.'
    for cell in row:
        if cell is not None and 'JOB MASTER' in str(cell).upper():
            return True
    return False
...
def parse_press_section(section_rows, offset, sheet_name, press_name):
    ...
    for local_i in range(data_start_local, len(section_rows)):
        ...
        if is_summary_row(jm_str, start_str_early, finish_str_early) or is_summary_row(jn_str, start_str_early, finish_str_early):
            pass # We will handle it below by setting row_type = 'summary'
        else:
        ...
        item = {
            ...
            'row_type':     row_type,
            'job_master':   str(gv('job_master', '') or ''),
...
def choose_sheets(wb, target_shift_req):
    ...
    If target_shift_req == 'AUTO', process all shifts (Pagi + Malam), preferring Rev variants.
    """
    global sheetnames
    sheetnames = wb.sheetnames

    def best_sheet(base):
        """Return the best available sheet name for a base shift name."""
        rev = f"{base} (Rev)"
        # Also check variants like 'Shift Pagi Revisi', 'Shift Pagi (Revisi)', etc.
        rev_variants = [rev]
        for sn in sheetnames:
            su = sn.upper()
    ...
    if target_shift_req != 'AUTO':
        ...
        chosen = best_sheet(target_shift_req)
        if chosen:
            return [chosen]
