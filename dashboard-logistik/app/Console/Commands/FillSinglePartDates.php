...
class FillSinglePartDates extends Command
{
    protected $signature   = 'singlepart:fill-dates {--from= : Start date Y-m-d} {--to= : End date Y-m-d}';
    protected $description = 'Carry-over data single part ke semua tanggal yang kosong';
    ...
    public function handle()
    {
        ...
        foreach ($this->typoMap as $wrong => $correct) {
            $rows = SinglePart::where('sheet_date', 'LIKE', "% {$wrong}")->get();
            foreach ($rows as $row) {
                $correctDate = str_replace(" {$wrong}", " {$correct}", $row->sheet_date);
                // Pastikan tidak duplikat
                $exists = SinglePart::where('sheet_date', $correctDate)
                    ->where('job_no', $row->job_no)->exists();
                if (!$exists) {
                    SinglePart::where('id', $row->id)->update(['sheet_date' => $correctDate]);
                    $fixed++;
                } else {
                    $row->delete();
                }
        ...
        $allSheets = SinglePart::distinct()->orderBy('sheet_date')->pluck('sheet_date');
        ...
        $existingSheets = SinglePart::distinct()->pluck('sheet_date')->mapWithKeys(function($s) {
            return [$s => true];
        })->toArray();
    ...
    {
        $oldItems = SinglePart::where('sheet_date', $fromSheet)->get();
