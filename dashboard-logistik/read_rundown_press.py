...
def get_status(strength):
    if strength <= 0:    return 'CRITICAL'
    elif strength < 1:   return 'MINIM'
    elif strength < 2:   return 'LIMITED'
...
def main():
    ...
    try:
        ...
        # Fallback to known positions (0-based) from analysis
        if col_no is None:          col_no = 1
        if col_job_sched is None:   col_job_sched = 2
        if col_job_deliv is None:   col_job_deliv = 3
        if col_job_stamp is None:   col_job_stamp = 4
        if col_keterangan is None:  col_keterangan = 5
        if col_variant is None:     col_variant = 6
        if col_pcs_day is None:     col_pcs_day = 12
        if col_cust is None:        col_cust = 13
        if col_qty_kbn is None:     col_qty_kbn = 14
        if col_maker is None:       col_maker = 15
        if col_stock_awal is None:  col_stock_awal = 16
        ...
        for label, cols in date_groups.items():
            ...
            for row in all_rows[data_start:]:
                ...
                if not re.match(r'^[A-Z]{1,4}[-_]?\d', job_str) and not re.match(r'^[A-Z0-9]{2,10}$', job_str):
                    continue
                if len(job_str) < 3 or len(job_str) > 20:
                    continue
                ...
                no_val        = gcol(col_no)
                job_delivery  = gcol(col_job_deliv)
                job_stamping  = gcol(col_job_stamp)
                keterangan    = gcol(col_keterangan)
                variant       = gcol(col_variant)
                pcs_day       = safe_float(gcol(col_pcs_day))
                cust          = gcol(col_cust)
                qty_kbn       = safe_float(gcol(col_qty_kbn))
                maker         = gcol(col_maker)
                stock_awal    = safe_float(gcol(col_stock_awal))

                mdfo          = safe_float(gcol(cols['mdfo']))
                order         = safe_float(gcol(cols['order']))
                plan_day      = safe_float(gcol(cols['plan_day']))
                plan_night    = safe_float(gcol(cols['plan_night']))
                act_prod      = safe_float(gcol(cols['act_prod']))
                stok_akhir    = safe_float(gcol(cols['stok_akhir']))
                strength      = safe_float(gcol(cols['strength']))
