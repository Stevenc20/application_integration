...
@section('content')
    ...
    <div class="hero">
        <div class="hero-title-block">
            <h2><span class="material-icons">people</span> Customer</h2>
            <div class="hero-meta">Kelola data master customer, detail kontak, email, dan status keaktifan</div>
        </div>
    </div>
    ...
    <div class="content-body">
        <div class="card">
            ...
            <div class="card-header">
                ...
                <form action="{{ route('customers.index') }}" method="GET" style="display:flex; align-items:center; gap:8px; flex-wrap:wrap; flex:1;">
                    ...
                    @if($search)
                    <a href="{{ route('customers.index') }}" class="btn-search-reset">Kembali</a>
                    @endif
                </form>

                {{-- Right: Actions buttons --}}
                <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;">
                    <a href="{{ route('customers.export', ['search' => $search]) }}" class="action-btn btn-green">
                        <span class="material-icons">file_download</span> Export Excel
                    </a>
                    <a href="{{ route('customers.template') }}" class="action-btn btn-outline">
                    ...
                    <a href="{{ route('customers.print_pdf', ['search' => $search]) }}" class="action-btn btn-red">
            ...
            <div class="table-wrap">
                <table>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td style="text-align: center;">{{ $customers->firstItem() + $index }}</td>
                            <td style="text-align: center;">
                                <a href="javascript:void(0)" class="customer-code-link" onclick="showDetail({{ json_encode($customer) }})">
                                    {{ $customer->kode }}
                                </a>
                            </td>
                            <td style="font-weight: 700; color: #333; text-align: center;">{{ $customer->nama }}</td>
                            <td style="text-align: center;">{{ $customer->kontak ?? '-' }}</td>
                            <td style="text-align: center;">{{ $customer->email ?? '-' }}</td>
                            <td style="text-align: center;">{{ $customer->telepon ?? '-' }}</td>
                            <td style="text-align: center;">
                                <span class="pill-badge {{ $customer->status === 'Aktif' ? 'pill-active' : 'pill-inactive' }}">
                                    {{ $customer->status }}
                                </span>
                            </td>
                            <td style="text-align: center;">
                                <div class="action-links" style="justify-content: center;">
                                    <a href="javascript:void(0)" class="link-detail" onclick="showDetail({{ json_encode($customer) }})">Detail</a>
                                    <a href="javascript:void(0)" class="link-edit" onclick="showEdit({{ json_encode($customer) }})">Edit</a>
                                    <form action="{{ route('customers.destroy', $customer->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus customer {{ $customer->nama }}?');" style="display:inline;">
                                        @csrf
                                        @method('DELETE')
            ...
            <div class="pagination-wrap">
                ...
                <div class="pagination">
                    ...
                    @else
                        <a href="{{ $customers->previousPageUrl() }}" class="page-btn"><span class="material-icons">chevron_left</span></a>
                    @endif
                    ...
                    @if($end < $customers->lastPage())
                        @if($end < $customers->lastPage() - 1)
                            <span style="padding:0 4px;color:#bbb;font-size:12px">...</span>
                        @endif
                        <a href="{{ $customers->url($customers->lastPage()) }}" class="page-btn">{{ $customers->lastPage() }}</a>
                    @endif

                    {{-- Next Page Link --}}
                    @if($customers->hasMorePages())
                        <a href="{{ $customers->nextPageUrl() }}" class="page-btn"><span class="material-icons">chevron_right</span></a>
                    @else
    ...
    <div class="modal-overlay" id="addModal">
        <div class="modal">
            <h3><span class="material-icons">add_circle</span> Tambah Customer Baru</h3>
            <form action="{{ route('customers.store') }}" method="POST">
                @csrf
                <div class="form-row">
                    <div class="form-group">
                        <label for="add_kode">Kode Customer <span style="color: red;">*</span></label>
                        <input type="text" name="kode" id="add_kode" placeholder="Contoh: C-ADM" required>
                    </div>
                    <div class="form-group">
                ...
                <div class="form-group" style="margin-bottom: 12px;">
                    <label for="add_nama">Nama Customer <span style="color: red;">*</span></label>
                    <input type="text" name="nama" id="add_nama" placeholder="Contoh: Astra Daihatsu Motor" required>
                </div>

                <div class="form-group" style="margin-bottom: 12px;">
                    <label for="add_kontak">Contact Person</label>
                    <input type="text" name="kontak" id="add_kontak" placeholder="Nama kontak penanggung jawab">
                </div>

                <div class="form-row" style="margin-bottom: 12px;">
                    <div class="form-group">
                        <label for="add_email">Email</label>
                        <input type="email" name="email" id="add_email" placeholder="customer@mail.com">
                    </div>
                    <div class="form-group">
                        <label for="add_telepon">Telepon</label>
                        <input type="text" name="telepon" id="add_telepon" placeholder="Nomor telepon aktif">
                    </div>
                </div>
    ...
    <div class="modal-overlay" id="editModal">
        <div class="modal">
            <h3><span class="material-icons">edit</span> Edit Customer</h3>
            <form action="{{ route('customers.update') }}" method="POST">
                @csrf
                <input type="hidden" name="id" id="edit_id">
                <div class="form-row">
                    <div class="form-group">
                        <label for="edit_kode">Kode Customer <span style="color: red;">*</span></label>
                ...
                <div class="form-group" style="margin-bottom: 12px;">
                    <label for="edit_nama">Nama Customer <span style="color: red;">*</span></label>
    ...
    <div class="modal-overlay" id="detailModal">
        <div class="modal">
            <h3><span class="material-icons">info</span> Detail Customer</h3>
            <div style="margin-bottom: 20px;">
                <table class="detail-table">
                    <tr>
                        <td class="label-td">Kode Customer</td>
                        <td class="value-td" id="detail_kode" style="font-family: monospace; color: var(--red-main);"></td>
                    </tr>
                    <tr>
                        <td class="label-td">Nama Customer</td>
