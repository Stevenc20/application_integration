...
.proses-stamping{background:#fef9c3;color:#854d0e;}.proses-subcont{background:#ede9fe;color:#7c3aed;}.proses-subassy{background:#e0f2fe;color:#0369a1;}.proses-rawmaterial{background:#fee2e2;color:#991b1b;}
...
@section('content')
    ...
    <form method="GET" action="{{ route('rundown.index') }}" id="filterForm">
        ...
        <div class="toolbar">
            ...
            <a href="{{ route('rundown.index') }}" class="btn-reset">Kembali</a>
    ...
    <div class="table-wrap" id="tableWrapRundown">
        ...
        <table>
            ...
            <tbody>
                ...
                if ($rem === 'ok' || $rem === 'normal') {
                    $remCls = 'badge-ok';
                    $qtyColor = '#16a34a';
                } elseif ($rem === 'minim' || $rem === 'rendah') {
                    $remCls = 'badge-minim';
                    $qtyColor = '#d97706';
                } else {
                    $remCls = 'badge-kosong';
                ...
                <tr>
                    ...
                    <td style="font-weight:700;color:#333">{{ $item->part_number }}</td>
                    <td>RM</td>
                    <td>{{ $locName }}</td>
                    <td style="text-align: right; font-weight:700; color: {{ $qtyColor }}">{{ number_format($item->stock_fg, 0) }}</td>
                    <td style="text-align: center; color: #999;">
                        {{ $qtyVendor > 0 ? number_format($qtyVendor, 0) : '—' }}
                    </td>
                    <td style="text-align: center;">{{ $uom }}</td>
                    <td style="text-align: right; color:#888;">{{ number_format($item->min_stock, 0) }}</td>
        ...
        <table>
            ...
            <tbody>
                ...
                } elseif (str_contains($pl, 'raw')) {
                    $prosesCls = 'proses-rawmaterial';
                } else {
