...
<style>
        ...
        .inline-input { width: 100px; border: 1.5px solid #ddd; border-radius: 6px; padding: 5px 8px; font-size: 11px; font-weight: 600; font-family: inherit; text-align: right; outline: none; transition: all .15s; }
    ...
    <div class="content-body">
        <div class="card">
            <div class="card-header" style="padding: 20px;">
                <form action="{{ route('rundown_incoming.index') }}" method="GET" id="toolbarForm" style="width:100%;">
                    <!-- ROW 1: Search, Kembali, and Actions (Delete, Add, Export, Upload) -->
                    <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:12px; margin-bottom:12px; border-bottom: 1px solid #eee; padding-bottom:12px;">
                        <!-- Left: Search Box, Cari, Kembali -->
                        <div style="display:flex; align-items:center; gap:12px; flex-wrap:wrap;">
                            ...
                            <a href="{{ route('rundown_incoming.index', ['sheet' => $selectedSheet]) }}" class="action-btn" style="background:#f1f5f9; color:#475569; border:1px solid #cbd5e1; height: 38px; display:flex; align-items:center; justify-content:center;">Kembali</a>
                        </div>
                        ...
                        <div style="display:flex; align-items:center; gap:10px; flex-wrap:wrap;">
                            ...
                            <div style="display:flex; border-right: 1px solid #eee; padding-right: 10px; gap: 8px;">
                                <a href="{{ route('rundown_incoming.template') }}" class="action-btn btn-green" style="text-decoration: none;"><span class="material-icons">file_download</span> Download Template</a>
                ...
                <table>
                    <thead>
                        <tr>
                            ...
                            <th>{{ $filterCategory === 'SINGLE PART' ? 'JOB NO FINISH' : 'JOB NO FINISH' }}</th>
                            <th>TYPE PALLET</th>
                            <th>KATEGORI</th>
                            <th>CUSTOMER</th>
                    ...
                    <tbody>
                        ...
                        <tr id="row-{{ $item->id }}">
                            ...
                            <td style="color:#666;font-size:11px;">
                                @if($isFinishPart && $relatedSingleParts->has($searchKey))
                                    <div style="cursor:pointer; display:flex; align-items:center; gap:4px; padding:4px 0; font-weight:700; color:var(--navy-main);" onclick="toggleSingleParts('{{ $item->id }}')">
                                        <span>{{ $item->job_no_finish ?: count($relatedSingleParts[$searchKey]) . ' Parts' }}</span>
                                        <span class="material-icons" id="icon-toggle-{{ $item->id }}" style="font-size:16px; color:var(--red-main); background:#fee2e2; border-radius:50%;">expand_more</span>
                                    </div>
                                @else
                                    {{ $item->job_no_finish }}
                                @endif
                            ...
                            <td style="text-align:right">
                                <input type="number" class="inline-input price-input-{{ $item->id }}" style="width: 100px;" data-id="{{ $item->id }}" data-field="price_pc" value="{{ number_format($item->price_pc, 0, '.', '') }}" onchange="saveInline(this)" step="1">
                            </td>
                        ...
                        {{-- Collapsible Row for Single Parts --}}
                        @if($isFinishPart && $relatedSingleParts->has($searchKey))
                        <tr id="sp-row-{{ $item->id }}" style="display:none; background:#fafafa;">
                            <td colspan="23" style="padding: 12px 24px; border-bottom: 2px solid #eaeaea;">
                                <div style="background:white; border:1px solid #e2e8f0; border-radius:8px; padding:16px; box-shadow:0 4px 6px -1px rgba(0, 0, 0, 0.05); overflow-x: auto;">
                                    <div style="font-size:12px; font-weight:800; color:var(--navy-dark); margin-bottom:12px; display:flex; align-items:center; gap:8px; text-transform:uppercase; letter-spacing:0.5px;">
                                        <span class="material-icons" style="font-size:16px; color:var(--red-main);">subdirectory_arrow_right</span>
                                        Komponen Single Part untuk {{ $item->job_no }}
                                    </div>
                                    <table style="min-width:1500px; width:100%; border:1px solid #f1f5f9; border-radius:6px; overflow:hidden;">
                                        <thead>
                                            <tr style="background:#f8fafc; border-bottom:1px solid #e2e8f0;">
                                                <th style="width:36px; padding:8px 12px; font-size:10px; color:#64748b; font-weight:700;">#</th>
                                                <th style="padding:8px 12px; font-size:10px; color:#64748b; font-weight:700;">JOB NO</th>
                                                <th style="padding:8px 12px; font-size:10px; color:#64748b; font-weight:700;">JOB NO FINISH</th>
                                                <th style="padding:8px 12px; font-size:10px; color:#64748b; font-weight:700;">TYPE PALLET</th>
                                                <th style="padding:8px 12px; font-size:10px; color:#64748b; font-weight:700;">KATEGORI</th>
                                                <th style="padding:8px 12px; font-size:10px; color:#64748b; font-weight:700;">CUSTOMER</th>
                                                <th style="padding:8px 12px; font-size:10px; color:#64748b; font-weight:700; text-align:right;">PRICE/PC</th>
                                                <th style="padding:8px 12px; font-size:10px; color:#64748b; font-weight:700;">VENDOR</th>
                                                <th style="padding:8px 12px; font-size:10px; color:#64748b; font-weight:700; text-align:center;">STATUS</th>
                                                <th style="padding:8px 12px; font-size:10px; color:#64748b; font-weight:700; text-align:center;">MOVEMENT</th>
                                                <th style="padding:8px 12px; font-size:10px; color:#64748b; font-weight:700; text-align:center;">CYCLE ISSUE</th>
                                                <th style="padding:8px 12px; font-size:10px; color:#64748b; font-weight:700; text-align:right;">STOCK AWAL ✏️</th>
                                                <th style="padding:8px 12px; font-size:10px; color:#64748b; font-weight:700; text-align:right;">INCOMING ✏️</th>
                                                <th style="padding:8px 12px; font-size:10px; color:#64748b; font-weight:700; text-align:right;">ASSY ✏️</th>
                                                <th style="padding:8px 12px; font-size:10px; color:#64748b; font-weight:700; text-align:right;">IAMI</th>
                                                <th style="padding:8px 12px; font-size:10px; color:#64748b; font-weight:700; text-align:right;">GKD</th>
                                                <th style="padding:8px 12px; font-size:10px; color:#64748b; font-weight:700; text-align:right;">SAP</th>
                                                <th style="padding:8px 12px; font-size:10px; color:#64748b; font-weight:700; text-align:right;">KAP</th>
                                                <th style="padding:8px 12px; font-size:10px; color:#64748b; font-weight:700; text-align:right;">IKAR/TMMIN/FTI</th>
                                                <th style="padding:8px 12px; font-size:10px; color:#64748b; font-weight:700; text-align:right;">STOK AKHIR</th>
                                                <th style="padding:8px 12px; font-size:10px; color:#64748b; font-weight:700; text-align:right;">ALL PRICE</th>
                                                <th style="padding:8px 12px; font-size:10px; color:#64748b; font-weight:700; text-align:right;">PCS/DAY ✏️</th>
                                                <th style="padding:8px 12px; font-size:10px; color:#64748b; font-weight:700; text-align:right;">STRENGTH</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            @foreach($relatedSingleParts[$searchKey] as $spIdx => $sp)
                                            <tr style="border-bottom:1px solid #f1f5f9; background:white;">
                                                <td style="padding:8px 12px; color:#999; font-size:11px;">{{ (($items->currentPage()-1)*$perPage + $i + 1) . '.' . ($spIdx + 1) }}</td>
                                                <td style="padding:8px 12px; font-weight:700; color:var(--navy-main); font-size:11px;">{{ $sp->job_no }}</td>
                                                <td style="padding:8px 12px; color:#64748b; font-size:11px;">{{ $sp->job_no_finish }}</td>
                                                <td style="padding:8px 12px; color:#64748b; font-size:11px;">{{ $sp->type_pallet ?: '-' }}</td>
                                                <td style="padding:8px 12px; font-size:11px;">
                                                    <span style="background:#e3f2fd;color:#1565c0;border:1px solid #90caf9;border-radius:6px;padding:3px 8px;font-size:10px;font-weight:700;">SINGLE PART</span>
                                                </td>
                                                <td style="padding:8px 12px; color:#64748b; font-size:11px;">{{ $sp->customer ?: '-' }}</td>
                                                <td style="padding:8px 12px; text-align:right;">
                                                    <input type="number" class="inline-input" data-id="{{ $sp->id }}" data-field="price_pc" value="{{ number_format($sp->price_pc, 0, '.', '') }}" onchange="saveInline(this)" step="1">
                                                </td>
                                                <td style="padding:8px 12px; color:#64748b; font-size:11px; font-weight:600;">{{ $sp->vendor }}</td>
                                                <td style="padding:8px 12px; text-align:center;" id="status-{{ $sp->id }}">
                                                    @php $spStClass = strtolower($sp->status ?: 'standar'); @endphp
                                                    <span class="badge-status {{ $spStClass }}" style="font-size:9px; padding:3px 8px; min-width:60px;">{{ $sp->status }}</span>
                                                </td>
                                                <td style="padding:8px 12px; text-align:center;">
                                                    <span class="badge-cat" style="font-size:9px;">{{ $sp->movement }}</span>
                                                </td>
                                                <td style="padding:8px 12px; text-align:center;">
                                                    <input type="number" class="inline-input" style="width:50px;text-align:center;font-weight:700;" data-id="{{ $sp->id }}" data-field="cycle_issue" value="{{ $sp->cycle_issue }}" onchange="saveInline(this)">
                                                </td>
                                                <td style="padding:8px 12px; text-align:right;">
                                                    <input type="number" class="inline-input" data-id="{{ $sp->id }}" data-field="stock_awal" value="{{ number_format($sp->stock_awal, 0, '.', '') }}" onchange="saveInline(this)">
                                                </td>
                                                <td style="padding:8px 12px; text-align:right;">
                                                    <input type="number" class="inline-input" data-id="{{ $sp->id }}" data-field="incoming" value="{{ number_format($sp->incoming, 0, '.', '') }}" onchange="saveInline(this)">
                                                </td>
                                                <td style="padding:8px 12px; text-align:right;">
                                                    <input type="number" class="inline-input" data-id="{{ $sp->id }}" data-field="assy" value="{{ number_format($sp->assy, 0, '.', '') }}" onchange="saveInline(this)">
                                                </td>
                                                
                                                @foreach(['iami','gkd','sap','kap','gmo'] as $coField)
                                                <td style="padding:8px 12px; text-align:right;">
                                                    <span style="display:inline-block;width:80px;text-align:right;font-size:11px;font-weight:600;color:#aaa;background:#f8f8f8;border:1px dashed #ddd;border-radius:6px;padding:5px 8px;">{{ number_format($sp->$coField, 0, ',', '.') }}</span>
                                                </td>
                                                @endforeach

                                                <td style="padding:8px 12px; text-align:right; font-weight:800; font-size:11px; color:{{$sp->stok_akhir<0?'#ef4444':'inherit'}}" id="stok-{{ $sp->id }}">{{ number_format($sp->stok_akhir, 0, ',', '.') }}</td>
                                                <td style="padding:8px 12px; text-align:right; font-size:11px;" id="allprice-{{ $sp->id }}">Rp {{ number_format($sp->all_price, 0, ',', '.') }}</td>
                                                <td style="padding:8px 12px; text-align:right;">
                                                    <input type="number" class="inline-input" data-id="{{ $sp->id }}" data-field="pcs_day" value="{{ number_format($sp->pcs_day, 0, '.', '') }}" step="0.01" onchange="saveInline(this)">
                                                </td>
                                                <td style="padding:8px 12px; text-align:right; font-size:11px; font-weight:700; color:{{$sp->strength < 2 ? '#dc2626' : ($sp->strength >= 5 ? '#3b82f6' : '#16a34a')}}" id="str-{{ $sp->id }}">{{ number_format($sp->strength, 2) }}</td>
                                            </tr>
                                            @endforeach
                                        </tbody>
                                    </table>
    ...
    <div class="modal-overlay" id="modalOverlay" onclick="closeModalOutside(event)">
        <div class="modal">
            ...
            <form action="{{ route('rundown_incoming.add') }}" method="POST">
                ...
                <div class="form-row">
                    <div class="form-group">
                        <label>Job No Finish</label>
                        <input type="text" name="job_no_finish" placeholder="e.g. SC-0743">
                    </div>
                    <div class="form-group">
                        <label>Type Pallet</label>
                        <input type="text" name="type_pallet" placeholder="e.g. TP-332">
                    </div>
                </div>
                <div class="form-row">
                    <div class="form-group">
...
function toggleSingleParts(id) {
    ...
    if (row.style.display === 'none') {
        ...
        icon.style.background = 'var(--red-main)';
        icon.style.color = 'white';
    } else {
        row.style.display = 'none';
        icon.innerText = 'expand_more';
        icon.style.background = '#fee2e2';
        icon.style.color = 'var(--red-main)';
    }
}
...
function saveInline(input) {
    ...
    .then(data => {
        if(data.success) {
            ...
            // If a parent Finish Part was also updated, refresh its UI values
            if (data.parent) {
                let pId = data.parent.id;
                let pInput = document.querySelector('.price-input-' + pId);
                if (pInput) {
                    pInput.value = data.parent.price_pc;
                }
                let pAllPrice = document.getElementById('allprice-' + pId);
                if (pAllPrice) {
                    pAllPrice.textContent = formatRp(data.parent.all_price);
                }
            }
        }
    })
