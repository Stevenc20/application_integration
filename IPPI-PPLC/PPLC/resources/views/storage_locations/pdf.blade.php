...
<head>
    <meta charset="utf-8">
    <title>Daftar Storage Location PDF</title>
    <style>
        body {
...
<body>
    ...
    <div class="meta-info" style="border-bottom: 1px dashed #cbd5e1; padding-bottom: 4px; margin-bottom: 8px;">
        <strong>Filter aktif:</strong> {{ $filterStr }} &nbsp;|&nbsp; <strong>Total:</strong> {{ $locations->count() }} storage location
    </div>
    ...
    <table class="table-data">
        <thead>
            <tr>
                ...
                <th style="width: 9%; text-align: center;">SCRAP</th>
            </tr>
        </thead>
        <tbody>
            ...
            <tr>
                ...
                <td class="code-td">{{ $location->kode }}</td>
                <td style="font-weight: bold;">{{ $location->nama }}</td>
                <td>{{ $location->deskripsi ?? '-' }}</td>
                <td>{{ $location->tipe_material }}</td>
                <td style="text-align: center; font-weight: bold; color: {{ $location->is_scrap ? '#dc2626' : '#666' }};">
                    {{ $location->is_scrap ? 'Scrap' : '—' }}
                </td>
            </tr>
            @endforeach
        </tbody>
    ...
    <div class="footer">
        <table class="meta-flex">
            <tr>
                <td>
                    Total {{ $locations->count() }} storage location ditampilkan.
                </td>
