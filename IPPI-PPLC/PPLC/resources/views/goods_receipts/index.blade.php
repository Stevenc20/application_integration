...
<style>
    ...
    .pill-drafted { background: #e2e8f0; color: #475569; }
...
@section('content')
    ...
    <div class="hero">
        <div class="hero-title-block">
            <h2><span class="material-icons">inventory_2</span> Goods Receipt</h2>
            <div class="hero-meta">Kelola penerimaan pasokan barang masuk dari Purchase Order dan penempatan stok pada storage location</div>
        </div>
    </div>
        ...
        <div class="card">
            ...
            <div class="card-header" style="flex-direction: column; align-items: stretch; gap: 14px;">
                <div style="display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:12px;">
                    <div style="font-size: 15px; font-weight: 800; color: var(--navy-dark);">Goods Receipt</div>
                    <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;">
                        <a href="{{ route('goods_receipts.export', ['search' => $search, 'start_date' => $startDate, 'end_date' => $endDate, 'vendor_id' => $vendorId, 'location_id' => $locationId]) }}" class="action-btn btn-green">
                            <span class="material-icons">file_download</span> Export Excel
                        </a>
                        <a href="{{ route('goods_receipts.print_pdf', ['search' => $search, 'start_date' => $startDate, 'end_date' => $endDate, 'vendor_id' => $vendorId, 'location_id' => $locationId]) }}" class="action-btn btn-red">
                        ...
                        <button type="button" class="action-btn btn-navy" style="background:var(--navy-dark);" onclick="openModal('addModal')">
                            <span class="material-icons">add</span> + Buat GR
                        </button>
                    </div>
                </div>

                {{-- Filters form matches the screenshot layout --}}
                <form action="{{ route('goods_receipts.index') }}" method="GET" class="filter-row">
                    <input type="text" name="search" value="{{ $search }}" class="filter-input" placeholder="No. GR / No. PO..." style="width: 200px;">
                    ...
                    @if($search || $startDate || $endDate || ($vendorId && $vendorId !== 'Semua Vendor') || ($locationId && $locationId !== 'Semua Lokasi'))
                        <a href="{{ route('goods_receipts.index') }}" class="btn-search-reset">Kembali</a>
                    @endif
                </form>
            </div>
            ...
            <div class="table-wrap">
                <table>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>
                                <a href="javascript:void(0)" class="gr-code-link" onclick="showDetail({{ $gr->id }})">
                                    {{ $gr->no_gr }}
                                </a>
                            </td>
                            <td style="font-family: monospace; font-weight: 700;">{{ $gr->purchaseOrder->no_po ?? '-' }}</td>
                            <td style="font-weight: 700; color: #333;">{{ $gr->purchaseOrder->vendor->nama ?? '-' }}</td>
                            <td>{{ $gr->tanggal_terima->format('d/m/Y') }}</td>
                            <td>{{ $gr->storageLocation->nama ?? '-' }}</td>
                            <td>
                                @if($gr->status === 'drafted')
                                    <span class="pill-badge pill-drafted">drafted</span>
                                @elseif($gr->status === 'posted')
                                    <span class="pill-badge pill-posted">posted</span>
                                @else
                                    <span class="pill-badge">{{ $gr->status }}</span>
                                @endif
                            </td>
                            <td style="text-align: center;">
                                <div class="action-links" style="justify-content: center;">
                                    <a href="javascript:void(0)" class="link-detail" onclick="showDetail({{ $gr->id }})">Detail</a>
                                    <a href="javascript:void(0)" class="link-edit" onclick="showEdit({{ $gr->id }})">Edit</a>
                                    <form action="{{ route('goods_receipts.destroy', $gr->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus GR {{ $gr->no_gr }}?');" style="display:inline;">
                                        @csrf
                                        @method('DELETE')
            ...
            {{-- Pagination --}}
            @if($goodsReceipts->total() > 0 && $goodsReceipts->lastPage() > 1)
            <div class="pagination-wrap">
                <div class="pagination-info">
                    Menampilkan <strong>{{ $goodsReceipts->firstItem() ?? 0 }}-{{ $goodsReceipts->lastItem() ?? 0 }}</strong> dari <strong>{{ $goodsReceipts->total() }}</strong> Goods Receipt
                </div>
                
                <div class="pagination">
                    {{-- Previous Page Link --}}
                    @if($goodsReceipts->onFirstPage())
                        <span class="page-btn disabled"><span class="material-icons">chevron_left</span></span>
                    @else
                        <a href="{{ $goodsReceipts->previousPageUrl() }}" class="page-btn"><span class="material-icons">chevron_left</span></a>
                    @endif

                    {{-- Pagination Pages --}}
                    @php
                        $start = max(1, $goodsReceipts->currentPage() - 2);
                        $end = min($goodsReceipts->lastPage(), $goodsReceipts->currentPage() + 2);
                    @endphp

                    @if($start > 1)
                        <a href="{{ $goodsReceipts->url(1) }}" class="page-btn">1</a>
                        @if($start > 2)
                    ...
                    @for($page = $start; $page <= $end; $page++)
                        @if($page == $goodsReceipts->currentPage())
                            <span class="page-btn active">{{ $page }}</span>
                        @else
                            <a href="{{ $goodsReceipts->url($page) }}" class="page-btn">{{ $page }}</a>
                        @endif
                    @endfor

                    @if($end < $goodsReceipts->lastPage())
                        @if($end < $goodsReceipts->lastPage() - 1)
                            <span style="padding:0 4px;color:#bbb;font-size:12px">...</span>
                        @endif
                        <a href="{{ $goodsReceipts->url($goodsReceipts->lastPage()) }}" class="page-btn">{{ $goodsReceipts->lastPage() }}</a>
                    @endif

                    {{-- Next Page Link --}}
                    @if($goodsReceipts->hasMorePages())
                        <a href="{{ $goodsReceipts->nextPageUrl() }}" class="page-btn"><span class="material-icons">chevron_right</span></a>
                    @else
    ...
    <div class="modal-overlay" id="addModal">
        <div class="modal">
            <h3><span class="material-icons">add_circle</span> Buat Goods Receipt Baru</h3>
            <form action="{{ route('goods_receipts.store') }}" method="POST">
                @csrf
                <div class="form-row">
                    <div class="form-group">
                        <label for="add_no_gr">Nomor GR <span style="color: red;">*</span></label>
                        <input type="text" name="no_gr" id="add_no_gr" placeholder="Contoh: GR-2026-00007" required>
                    </div>
                    <div class="form-group">
                        <label for="add_purchase_order_id">Purchase Order (Opsional)</label>
                        <select name="purchase_order_id" id="add_purchase_order_id" onchange="loadPOItemsForAdd(this.value)">
                ...
                <div class="form-row">
                    <div class="form-group">
                        <label for="add_tanggal_terima">Tanggal Terima <span style="color: red;">*</span></label>
                        <input type="date" name="tanggal_terima" id="add_tanggal_terima" required>
                    </div>
                    <div class="form-group">
                        <label for="add_storage_location_id">Storage Location <span style="color: red;">*</span></label>
                        <select name="storage_location_id" id="add_storage_location_id" required>
                ...
                <div class="items-container-box">
                    <div class="items-header">
                        ...
                        <button type="button" class="btn-add-item-row" onclick="addItemRow('add_items_list')">+ Tambah Baris</button>
                    </div>
                    <div id="add_items_list">
                        {{-- Rows appended here --}}
                        <div class="item-row-entry" id="add_row_0">
                            <select name="items[0][material_id]" required class="filter-select" style="min-width:100px;">
                                <option value="">Pilih Material...</option>
                                @foreach($materials as $material)
                                    <option value="{{ $material->id }}">{{ $material->kode }} - {{ $material->nama }}</option>
                                @endforeach
                            </select>
                            <input type="number" step="0.001" name="items[0][qty]" placeholder="Qty Diterima..." required class="filter-input" style="height:34px;">
                            <button type="button" class="btn-remove-row" onclick="removeRow('add_row_0')">
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" class="btn-save">Simpan GR</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div class="modal-overlay" id="editModal">
        <div class="modal">
            <h3><span class="material-icons">edit</span> Edit Goods Receipt</h3>
            <form action="{{ route('goods_receipts.update') }}" method="POST">
                @csrf
                <input type="hidden" name="id" id="edit_id">
                <div class="form-row">
                    <div class="form-group">
                        <label for="edit_no_gr">Nomor GR <span style="color: red;">*</span></label>
                        <input type="text" name="no_gr" id="edit_no_gr" required>
                    </div>
                    <div class="form-group">
                        <label for="edit_purchase_order_id">Purchase Order (Opsional)</label>
                        <select name="purchase_order_id" id="edit_purchase_order_id">
                            <option value="">Tanpa PO (Penerimaan Langsung)...</option>
                            @foreach($purchaseOrders as $po)
                                <option value="{{ $po->id }}">{{ $po->no_po }} ({{ $po->vendor->nama ?? '-' }})</option>
                            @endforeach
                ...
                <div class="form-row">
                    <div class="form-group">
                        <label for="edit_tanggal_terima">Tanggal Terima <span style="color: red;">*</span></label>
                        <input type="date" name="tanggal_terima" id="edit_tanggal_terima" required>
                    </div>
                    <div class="form-group">
                        <label for="edit_storage_location_id">Storage Location <span style="color: red;">*</span></label>
        ...
        <div class="modal">
            ...
            <div style="margin-bottom: 20px;">
                <table class="detail-table">
                    ...
                    <tr>
                        <td class="label-td">Nomor PO</td>
                        <td class="value-td" id="detail_no_po"></td>
                    </tr>
                    <tr>
                        <td class="label-td">Vendor</td>
                        <td class="value-td" id="detail_vendor"></td>
                    </tr>
                    <tr>
            ...
            <div class="items-container-box" style="background: white;">
                <div class="items-header">RINCIAN MATERIAL DITERIMA</div>
...
<script>
    ...
    // Store POs in JSON to access items client-side
    ...
    function addItemRow(containerId) {
        ...
        const rowHtml = `
            <div class="item-row-entry" id="${rowId}">
                ...
                <input type="number" step="0.001" name="items[${index}][qty]" placeholder="Qty Diterima..." required class="filter-input" style="height:34px;">
    ...
    // Auto-populate items based on selected PO
    function loadPOItemsForAdd(poId) {
        ...
        po.items.forEach(item => {
            ...
            const rowHtml = `
                <div class="item-row-entry" id="${rowId}">
                    <select name="items[${addRowIndex}][material_id]" required class="filter-select" style="min-width:100px;">
                        <option value="${item.material_id}" selected>${item.material.kode} - ${item.material.nama}</option>
                    </select>
                    <input type="number" step="0.001" name="items[${addRowIndex}][qty]" value="${remainingQty}" placeholder="Qty Diterima..." required class="filter-input" style="height:34px;">
                    <button type="button" class="btn-remove-row" onclick="removeRow('${rowId}')">
                        <span class="material-icons" style="font-size:16px;">delete</span>
                    </button>
                </div>
            ...
            .then(data => {
                ...
                data.items.forEach(item => {
                    ...
                    tr.innerHTML = `
                        ...
                        <td style="padding: 8px; text-align: right; font-weight: bold; color: #15803d;">${parseFloat(item.qty).toLocaleString('en-US', {minimumFractionDigits: 3, maximumFractionDigits: 3})}</td>
                ...
                data.items.forEach(item => {
                    ...
                    const rowHtml = `
                        <div class="item-row-entry" id="${rowId}">
                            ...
                            <input type="number" step="0.001" name="items[${editRowIndex}][qty]" value="${item.qty}" placeholder="Qty Diterima..." required class="filter-input" style="height:34px;">
                            <button type="button" class="btn-remove-row" onclick="removeRow('${rowId}')">
                                <span class="material-icons" style="font-size:16px;">delete</span>
                            </button>
                        </div>
    ...
    // Auto-open modal if po_id is in query string
