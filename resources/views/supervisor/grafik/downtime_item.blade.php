    ...
    <div class="p-5">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            ...
            <div class="border border-gray-200 rounded-lg p-4 bg-white shadow-sm text-center">
                ...
                <p class="text-2xl font-bold text-gray-800 mt-1" id="monthLabel">{{ date('F Y') }}</p>
            </div>
        </div>

        <div class="border border-gray-200 rounded-lg p-6 bg-white shadow-sm">
            <h6 class="font-bold text-gray-700 mb-4 text-center">Top Alasan Downtime (Menit)</h6>
...
async function loadData() {
    ...
    const res = await fetch("{{ route('supervisor.api.grafik.downtime_item') }}?" + params);
    const data = await res.json();

    document.getElementById('monthLabel').textContent = new Date(month + '-01').toLocaleDateString('id-ID', { year: 'numeric', month: 'long' });
