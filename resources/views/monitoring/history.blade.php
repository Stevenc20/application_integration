...
@php
    $title = match ($type) {
        ...
        'break' => 'History Break Time',
        default => 'History',
    };
    $icon = match ($type) {
        'downtime' => '<svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/></svg>',
        'break' => '<svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z"/></svg>',
    };
    $isEditable = in_array($type, ['downtime', 'break']);
@endphp
...
<div class="space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div class="flex items-center gap-2">
            <span class="text-primary-red">{!! $icon !!}</span>
    ...
    <div class="bg-amber-50 border-l-4 border-amber-400 border border-amber-200 rounded-xl p-4 flex items-start gap-3">
        ...
        <div>
            <p class="text-sm font-bold text-amber-800">Ada {{ $uneditedCount }} data downtime yang belum diisi detailnya.</p>
        ...
        <div>
            ...
            <div class="flex rounded-lg border border-gray-300 overflow-hidden">
                <button type="button" onclick="setShiftFilter(0)"
                    class="px-3 py-2 text-xs font-bold transition-all {{ $selectedShift === 0 ? 'bg-primary-red text-white' : 'bg-white text-gray-500 hover:bg-gray-50' }}">Semua</button>
                <button type="button" onclick="setShiftFilter(1)"
                    class="px-3 py-2 text-xs font-bold transition-all {{ $selectedShift === 1 ? 'bg-primary-red text-white' : 'bg-white text-gray-500 hover:bg-gray-50' }}">Shift Pagi</button>
                <button type="button" onclick="setShiftFilter(2)"
                    class="px-3 py-2 text-xs font-bold transition-all {{ $selectedShift === 2 ? 'bg-primary-red text-white' : 'bg-white text-gray-500 hover:bg-gray-50' }}">Shift Malam</button>
            </div>
            <input type="hidden" name="shift" id="shiftInput" value="{{ $selectedShift }}">
        </div>
        <button type="submit"
                class="px-4 py-2 bg-primary-red text-white rounded-lg text-sm font-bold hover:bg-red-700 transition">
            Filter
        </button>
    </form>
                    ...
                    <tr class="hover:bg-gray-50 transition {{ $isUnedited ? 'bg-red-50/30' : '' }}">
                        ...
                        <td class="px-4 py-3">
                            <span class="inline-block px-2 py-0.5 rounded-full text-xs font-bold
                                {{ $dt->jenis_downtime === 'break time' ? 'bg-indigo-100 text-indigo-700' : 'bg-red-100 text-red-700' }}">
                                {{ $dt->jenis_downtime }}
                            </span>
                            @if($isUnedited)
...
<div id="editDowntimeModal" class="fixed inset-0 bg-black/50 backdrop-blur-sm hidden z-[9999] items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg border border-gray-100 overflow-hidden">
        <div class="border-b border-gray-100 px-6 py-4 flex items-center justify-between bg-red-50">
            <div>
                <h3 class="font-bold text-red-800 text-lg">Edit Downtime</h3>
                <p class="text-sm text-red-600 mt-0.5">Lengkapi detail masalah downtime</p>
            </div>
            <a href="{{ url()->current() }}?type={{ $type }}&date={{ $selectedDate }}&line={{ $selectedLine }}&shift={{ $selectedShift }}" class="w-8 h-8 rounded-lg bg-white border border-red-200 text-red-400 hover:text-red-600 flex items-center justify-center transition-colors">
        ...
        <div class="p-6 space-y-4">
            ...
            <div class="flex gap-3 pt-2">
                <a href="{{ url()->current() }}?type={{ $type }}&date={{ $selectedDate }}&line={{ $selectedLine }}&shift={{ $selectedShift }}" id="editModalBatalBtn" class="flex-1 px-4 py-2.5 rounded-xl border border-gray-200 text-gray-600 hover:bg-gray-50 font-bold text-sm transition-all text-center">Batal</a>
                <button id="editModalSaveBtn" class="flex-1 px-4 py-2.5 rounded-xl bg-red-600 hover:bg-red-700 text-white font-bold text-sm shadow-lg shadow-red-100 transition-all flex items-center justify-center gap-2" type="button">
...
function deleteBreakEntry(id) {
    if (!confirm('Hapus entry break time ini?')) return;
