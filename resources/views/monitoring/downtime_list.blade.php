            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    @php
                        ...
                        $dtLower = strtolower($dt->jenis_downtime ?? '');
                        $badgeClass = match (true) {
                            str_contains($dtLower, 'machine') || str_contains($dtLower, 'mach') || str_contains($dtLower, 'mesin') => 'bg-blue-100 text-blue-700',
                            str_contains($dtLower, 'material') || str_contains($dtLower, 'mat') => 'bg-amber-100 text-amber-700',
                            str_contains($dtLower, 'dies') => 'bg-purple-100 text-purple-700',
                            str_contains($dtLower, 'logistic') || str_contains($dtLower, 'log') => 'bg-teal-100 text-teal-700',
                            str_contains($dtLower, 'production') || str_contains($dtLower, 'prod') => 'bg-emerald-100 text-emerald-700',
                            str_contains($dtLower, 'ubp') => 'bg-gray-100 text-gray-700',
                    ...
                    <tr class="hover:bg-gray-50 transition {{ $isUnedited ? 'bg-red-50/30' : '' }}">
                        ...
                        <td class="px-4 py-3">
                            <span class="inline-block px-2 py-0.5 rounded-full text-xs font-bold {{ $badgeClass }}">
                                {{ $dt->jenis_downtime }}
                            </span>
                            @if($isUnedited)
...
<div id="editDowntimeModal" class="fixed inset-0 bg-black/50 backdrop-blur-sm hidden z-[9999] items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg border border-gray-100 overflow-hidden">
        <div class="border-b border-gray-100 px-6 py-4 flex items-center justify-between bg-red-50">
            ...
            <a href="{{ url()->current() }}?date={{ $selectedDate }}&line={{ $selectedLine }}&shift={{ $selectedShift }}" class="w-8 h-8 rounded-lg bg-white border border-red-200 text-red-400 hover:text-red-600 flex items-center justify-center transition-colors">
...
function deleteBreakEntry(id) {
    if (!confirm('Hapus entry downtime ini?')) return;
