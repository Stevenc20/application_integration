...
<div class="p-4 sm:p-6 bg-gray-50 min-h-screen">
    ...
    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2 mb-6">
        <div>
            <h1 class="text-xl sm:text-2xl font-bold text-gray-800">Production Dashboard</h1>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        <div class="bg-white border border-gray-200 rounded-xl p-5 shadow-sm">
            ...
            <div class="space-y-3">
                @foreach($lineSummaries as $line => $summary)
                <div>
                    <div class="flex justify-between text-sm mb-1">
                        <span class="font-medium text-gray-700">{{ $line }}</span>
                        <span class="text-gray-500">{{ number_format($summary['ok']) }} / {{ number_format($summary['target']) }} ({{ $summary['achievement'] }}%)</span>
                    </div>
                    <div class="h-2 bg-gray-100 rounded-full overflow-hidden">
                        <div class="h-full rounded-full {{ $summary['achievement'] >= 100 ? 'bg-green-500' : ($summary['achievement'] >= 80 ? 'bg-yellow-500' : 'bg-red-500') }}" style="width: {{ min($summary['achievement'], 100) }}%"></div>
                    </div>
                </div>
                @endforeach
            </div>
