...
<div class="space-y-6">
    ...
    <div class="bg-gradient-to-r from-red-800 via-rose-700 to-red-600 rounded-3xl px-8 py-8 shadow-xl relative overflow-hidden flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div class="relative flex items-center gap-5">
            <div class="w-14 h-14 bg-white/15 backdrop-blur-sm rounded-2xl flex items-center justify-center text-white ring-1 ring-white/20 shadow-lg">
                <span class="material-icons text-3xl">factory</span>
            </div>
            <div>
                <h1 class="text-2xl font-black text-white tracking-tight">SMR Vendor</h1>
                <p class="text-rose-200 text-sm font-semibold mt-1">Data Supplier Quality SMR Vendor — Sheet SMR VENDOR</p>
            </div>
        </div>
        <div class="relative flex gap-3 flex-wrap">
            ...
            <div class="bg-white/10 backdrop-blur-sm rounded-xl px-5 py-3 text-center ring-1 ring-white/10 min-w-[100px] border-t-2 border-amber-400">
                ...
                <div class="text-[9px] font-bold text-white/60 uppercase tracking-wider mt-0.5">Qty Claim NG</div>
            </div>
            <div class="bg-white/10 backdrop-blur-sm rounded-xl px-5 py-3 text-center ring-1 ring-white/10 min-w-[100px] border-t-2 border-emerald-400">
                <div class="text-xl font-black text-white">{{ number_format($totalRepl) }}</div>
                <div class="text-[9px] font-bold text-white/60 uppercase tracking-wider mt-0.5">Qty Replacement</div>
            </div>
            <div class="bg-white/10 backdrop-blur-sm rounded-xl px-5 py-3 text-center ring-1 ring-white/10 min-w-[100px] border-t-2 border-red-400">
                <div class="text-xl font-black text-white">{{ number_format($uncloseCount) }}</div>
                <div class="text-[9px] font-bold text-white/60 uppercase tracking-wider mt-0.5">Open SMR</div>
            </div>
            <div class="bg-white/10 backdrop-blur-sm rounded-xl px-5 py-3 text-center ring-1 ring-white/10 min-w-[100px]">
                <div class="text-xl font-black text-white">{{ number_format($closeRate, 1) }}%</div>
                <div class="text-[9px] font-bold text-white/60 uppercase tracking-wider mt-0.5">Close Rate</div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col items-center justify-center py-20">
        ...
        <h3 class="text-lg font-black text-slate-400 mb-2">Belum ada data SMR Vendor</h3>
        <p class="text-sm text-slate-300 max-w-xs text-center leading-relaxed">Upload file Excel XLSM dari halaman Dashboard untuk memuat data SMR Vendor.</p>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col">
        ...
        {{-- Toolbar --}}
        <form method="GET" action="{{ route('smr_vendor.index') }}" id="filterForm" class="px-6 py-4 border-b border-slate-100">
            <div class="flex flex-wrap items-center gap-2">
                <div class="flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 flex-1 min-w-[160px] max-w-[240px] focus-within:border-rose-400 focus-within:ring-2 focus-within:ring-rose-100 transition-all">
                    ...
                    <input type="text" name="search" value="{{ $search }}" placeholder="Cari Part Name, No SMR, Problem..." class="bg-transparent border-none outline-none text-sm w-full font-medium text-slate-700 placeholder-slate-400" onchange="document.getElementById('filterForm').submit()">
                </div>
                ...
                <select name="status_barang" class="bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-sm font-medium text-slate-600 outline-none focus:border-rose-400 focus:ring-1 focus:ring-rose-400 h-[42px] cursor-pointer min-w-[140px]" onchange="document.getElementById('filterForm').submit()">
                ...
                <a href="{{ route('smr_vendor.index') }}" class="bg-slate-100 hover:bg-slate-200 text-slate-600 font-bold py-2.5 px-4 rounded-xl transition-all text-sm h-[42px] flex items-center">Reset</a>
        ...
        {{-- Table --}}
        <div class="table-wrap overflow-x-auto" id="tableWrapSmr">
            @if($items->isEmpty())
            <div class="flex flex-col items-center justify-center py-16 text-center">
                <span class="material-icons text-5xl text-slate-300 mb-3">search_off</span>
                <p class="text-sm font-medium text-slate-400">Tidak ada data SMR Vendor yang cocok</p>
            </div>
            @else
            <table class="w-full text-left border-collapse min-w-[1200px]">
                <thead>
                    <tr class="bg-slate-50 border-y border-slate-200">
                        ...
                        <th class="py-3 px-4 text-[10px] font-black text-slate-500 uppercase tracking-widest whitespace-nowrap">Tanggal Keluar</th>
                        <th class="py-3 px-4 text-[10px] font-black text-slate-500 uppercase tracking-widest whitespace-nowrap">Tanggal Masuk</th>
                        <th class="py-3 px-4 text-[10px] font-black text-slate-500 uppercase tracking-widest whitespace-nowrap text-right">Qty Pengganti</th>
                ...
                <tbody class="divide-y divide-slate-100 bg-white">
                    ...
                    $statusCls = strtolower($item->status_barang) === 'close' ? 'bg-emerald-100 text-emerald-700 border-emerald-200' : 'bg-red-100 text-red-700 border-red-200';
                    @endphp
                    <tr class="hover:bg-slate-50 transition-colors">
                        ...
                        <td class="py-3 px-4 text-xs font-black text-amber-500 text-right">{{ number_format($item->qty) }}</td>
                        <td class="py-3 px-4 text-xs text-slate-400 max-w-[220px] break-words">{{ $item->problem ?: '-' }}</td>
                        <td class="py-3 px-4 text-xs text-slate-500">{{ $item->tanggal_keluar ? $item->tanggal_keluar->format('d-m-Y') : '-' }}</td>
                        <td class="py-3 px-4 text-xs text-slate-500">{{ $item->tanggal_masuk ? $item->tanggal_masuk->format('d-m-Y') : '-' }}</td>
                        <td class="py-3 px-4 text-xs font-black text-emerald-500 text-right">{{ number_format($item->qty_pengganti) }}</td>
                        <td class="py-3 px-4"><span class="px-2.5 py-1 rounded-full text-[10px] font-black tracking-wider uppercase border {{ $statusCls }}">{{ $item->status_barang ?: '-' }}</span></td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
