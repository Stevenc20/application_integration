...
<ul class="list-none space-y-1 m-0 p-0">
    ...
    @if(auth()->user()->hasFeature('user_management') || auth()->user()->isSuperadmin() || auth()->user()->isAdmin())
    <li class="menu-item">
        <a href="{{ route('access-management.users.index') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 {{ request()->routeIs('access-management.users.*') ? 'bg-primary-red text-white shadow-md shadow-red-200' : 'text-gray-600 hover:bg-red-50 hover:text-primary-red' }}">
    ...
    <li class="menu-item">
        <a href="{{ route('access-management.features.index') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 {{ request()->routeIs('access-management.features.*') ? 'bg-primary-red text-white shadow-md shadow-red-200' : 'text-gray-600 hover:bg-red-50 hover:text-primary-red' }}">
            ...
            <span class="font-semibold">Role / Features</span>
        </a>
    </li>
    @endif
