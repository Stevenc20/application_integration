            ...
            <table class="min-w-full text-xs md:text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50/60 transition">
                        ...
                        <td class="px-5 py-4">
                            <span class="px-2 py-1 bg-blue-50 text-blue-600 rounded-lg text-xs font-semibold border border-blue-100 capitalize">
                                {{ $user->role }}
                            </span>
                        </td>
                        <td class="px-5 py-4 font-medium text-gray-700">
                            {{ $user->position ? $user->position->position_name : '-' }}
                        </td>
                        <td class="px-5 py-4 font-medium text-gray-700">
                        ...
                        <td class="px-5 py-4">
                            <div class="flex gap-2 justify-center">
                                <button type="button"
                                    ...
                                    data-user-position-id="{{ $user->position_id }}"
                                    data-user-section-id="{{ $user->section_id }}"
        ...
        <form action="{{ route('access-management.users.store') }}" method="POST" class="space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="position_id" class="w-full mt-1.5 px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-red-200 focus:border-red-400 outline-none transition">
                ...
                <div>
                    ...
                    <select name="section_id" class="w-full mt-1.5 px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-red-200 focus:border-red-400 outline-none transition">
                        <option value="">None</option>
    ...
    <div class="relative bg-white w-full max-w-lg mx-4 rounded-2xl shadow-2xl p-6">
        ...
        <form id="editForm" method="POST" class="space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="text-sm font-medium text-gray-700">Position</label>
                    <select name="position_id" id="edit_position_id" class="w-full mt-1.5 px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-red-200 focus:border-red-400 outline-none transition">
                        <option value="">None</option>
                        @foreach($positions as $p)
                        <option value="{{ $p->id }}">{{ $p->position_name }}</option>
                        @endforeach
                ...
                <div>
                    <label class="text-sm font-medium text-gray-700">Section</label>
                    <select name="section_id" id="edit_section_id" class="w-full mt-1.5 px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-red-200 focus:border-red-400 outline-none transition">
                        <option value="">None</option>
                        @foreach($sections as $s)
                        <option value="{{ $s->id }}">{{ $s->section_name }}</option>
                        @endforeach
...
window.openSuperAdminEditModal = function (btn) {
    ...
    var pos = btn.getAttribute('data-user-position-id');
    var sec = btn.getAttribute('data-user-section-id');

    document.getElementById('edit_name').value = name;
    document.getElementById('edit_nrp').value = nrp || '';
    document.getElementById('edit_role').value = role;
    document.getElementById('edit_position_id').value = pos || '';
    document.getElementById('edit_section_id').value = sec || '';
