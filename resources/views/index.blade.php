...
@section('content')
    ...
    <div class="hero">
        ...
        <div class="stat-cards">
            <div class="stat-card" style="background: #0f172a; min-width: 150px;">
                <span class="stat-card-label" style="color: rgba(255,255,255,0.75);">Total Plan Schedule Stamping</span>
                <span class="stat-card-value" style="font-size: 26px; display: block; margin-top: 4px;">{{ number_format($totalStampingPlan, 0, ',', '.') }}</span>
                @if($latestStampingDate)
                <span style="font-size: 8px; color: rgba(255,255,255,0.45); display: block; margin-top: 2px;">TGL: {{ $latestStampingDate }}</span>
                @endif
            </div>
            <div class="stat-card" style="background: #0f766e; min-width: 150px;">
                <span class="stat-card-label" style="color: rgba(255,255,255,0.75);">Kesiapan Material</span>
                <span class="stat-card-value" style="font-size: 26px; display: block; margin-top: 4px;">{{ $readyJobsCount }}/{{ $totalJobsCount }}</span>
                <span style="font-size: 8px; color: rgba(255,255,255,0.45); display: block; margin-top: 2px;">PER HARI INI</span>
            </div>
        </div>
    </div>
    ...
    {{-- CHART ANALYTICS SECTION (FINISH PART) --}}
    <div class="dashboard-body" style="padding-top:0; margin-top:-20px;">
        ...
        @if(!$hasFpData)
            <div style="grid-column: 1 / -1; background: #f9fafb; border: 1px dashed #ddd; border-radius: 12px; padding: 40px; text-align: center; color: #94a3b8; margin-top:20px;">
                ...
                <p style="font-size: 12px; margin-top: 4px;">Silakan isi data Rundown Stock kategori Finish Part terlebih dahulu.</p>
            </div>
        @else
    ...
    {{-- CHART ANALYTICS SECTION (SINGLE PART) --}}
    <div class="dashboard-body" style="padding-top:0; margin-top:20px;">
        <div class="chart-section-header">
            <span class="material-icons" style="color: var(--red-main); font-size: 28px;">analytics</span>
            <h2>Single Part Analytics</h2>
        </div>

        @if(!$hasSpData)
            <div style="grid-column: 1 / -1; background: #f9fafb; border: 1px dashed #ddd; border-radius: 12px; padding: 40px; text-align: center; color: #94a3b8; margin-top:20px;">
                <span class="material-icons" style="font-size: 48px; margin-bottom: 10px; opacity: 0.5;">bar_chart_off</span>
                <p style="font-size: 14px; font-weight: 600;">Tidak ada data Single Part untuk dianalisis.</p>
                <p style="font-size: 12px; margin-top: 4px;">Silakan isi data Rundown Stock kategori Single Part terlebih dahulu.</p>
            </div>
        @else
            <div class="chart-sum-grid">
                <div class="sum-card">
                    <div class="sum-icon blue"><span class="material-icons">inventory_2</span></div>
                    <div>
                        <div class="sum-val-lg">{{ $totalAllSingle }}</div>
                        <div class="sum-lbl">Total Item</div>
                    </div>
                </div>
                <div class="sum-card">
                    <div class="sum-icon green"><span class="material-icons">trending_up</span></div>
                    <div>
                        <div class="sum-val-lg" style="color:#16a34a">{{ $totalOverSP }}</div>
                        <div class="sum-lbl">Over Stock</div>
                    </div>
                </div>
                <div class="sum-card">
                    <div class="sum-icon yellow"><span class="material-icons">warning</span></div>
                    <div>
                        <div class="sum-val-lg" style="color:#d97706">{{ $totalLimitedSP }}</div>
                        <div class="sum-lbl">Limited Stock</div>
                    </div>
                </div>
                <div class="sum-card">
                    <div class="sum-icon red"><span class="material-icons">remove_circle</span></div>
                    <div>
                        <div class="sum-val-lg" style="color:#dc2626">{{ $totalZeroSP }}</div>
                        <div class="sum-lbl">Critical / Zero</div>
                    </div>
                </div>
            </div>
            ...
            <div class="charts-grid-dashboard">
                <div class="chart-card full">
                    <h3>Stock Level per Customer (Single Part)</h3>
                    <p>Jumlah item Over Stock, Limited, dan Zero per customer</p>
                    <div class="chart-container" style="height:300px">
                        <canvas id="chartCustomerSingle"></canvas>
                    </div>
                    <div class="legend">
                        <div class="legend-item"><div class="legend-dot" style="background:#22c55e"></div> Over Stock</div>
                        <div class="legend-item"><div class="legend-dot" style="background:#f59e0b"></div> Limited</div>
                ...
                <div class="chart-card">
                    <h3>Distribusi Status</h3>
                    <p>Perbandingan status item single part</p>
                    <div class="chart-container" style="height:280px">
                        <canvas id="chartRemarksSingle"></canvas>
                    </div>
                </div>

                <div class="chart-card">
                    <h3>Top 10 Vendor Distribution</h3>
                    <p>Jumlah item single part per vendor</p>
                    <div class="chart-container" style="height:280px">
                        <canvas id="chartProsesSingle"></canvas>
                    </div>
                </div>

                <div class="chart-card full">
                    <h3>Rata-rata Strength (Day) per Customer</h3>
                    <p>Berapa hari rata-rata stok single part bertahan per customer</p>
                    <div class="chart-container" style="height:260px">
                        <canvas id="chartStrengthSingle"></canvas>
                    </div>
                </div>
            </div>
        @endif
...
document.addEventListener('DOMContentLoaded', function() {
    const remarksColors = ['#22c55e','#f59e0b','#ef4444','#9333ea','#64748b','#059669','#92400e'];

    // --- FINISH PART CHARTS ---
    @if($hasFpData)
    (function() {
        const customerLabels = @json($fpPerCustomer->pluck('customer'));
        const overData       = @json($fpPerCustomer->pluck('over_stock')->map(fn($v) => (int)$v));
        const limitedData    = @json($fpPerCustomer->pluck('limited')->map(fn($v) => (int)$v));
        const zeroData       = @json($fpPerCustomer->pluck('zero_stock')->map(fn($v) => (int)$v));
        const remarksLabels  = @json($fpRemarksData->pluck('remarks'));
        const remarksCounts  = @json($fpRemarksData->pluck('total')->map(fn($v) => (int)$v));
        const prosesLabels   = @json($fpProsesData->pluck('proses'));
        const prosesCounts   = @json($fpProsesData->pluck('total')->map(fn($v) => (int)$v));
        const strengthLabels = @json($fpStrengthAvg->pluck('customer'));
        const strengthValues = @json($fpStrengthAvg->pluck('avg_strength')->map(fn($v) => round((float)$v, 2)));
    ...
    @if($hasSpData)
    (function() {
        const customerLabels = @json($spPerCustomer->pluck('customer'));
        const overData       = @json($spPerCustomer->pluck('over_stock')->map(fn($v) => (int)$v));
        const limitedData    = @json($spPerCustomer->pluck('limited')->map(fn($v) => (int)$v));
        const zeroData       = @json($spPerCustomer->pluck('zero_stock')->map(fn($v) => (int)$v));
        const remarksLabels  = @json($spRemarksData->pluck('remarks'));
        const remarksCounts  = @json($spRemarksData->pluck('total')->map(fn($v) => (int)$v));
        const prosesLabels   = @json($spProsesData->pluck('proses'));
        const prosesCounts   = @json($spProsesData->pluck('total')->map(fn($v) => (int)$v));
        const strengthLabels = @json($spStrengthAvg->pluck('customer'));
        const strengthValues = @json($spStrengthAvg->pluck('avg_strength')->map(fn($v) => round((float)$v, 2)));

        new Chart(document.getElementById('chartCustomerSingle'), {
            type: 'bar',
            data: {
                labels: customerLabels,
                datasets: [
        ...
        new Chart(document.getElementById('chartRemarksSingle'), {
            type: 'doughnut',
            data: { labels: remarksLabels, datasets: [{ data: remarksCounts, backgroundColor: remarksColors, borderWidth: 2, borderColor: '#fff' }] },
            options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'right', labels: { font: { size: 11 }, padding: 12 } } }, cutout: '60%' }
        });

        new Chart(document.getElementById('chartProsesSingle'), {
            type: 'bar',
            data: { labels: prosesLabels, datasets: [{ label: 'Jumlah Item', data: prosesCounts, backgroundColor: ['#C0001C', '#3b82f6', '#8b5cf6'], borderRadius: 8 }] },
            options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { x: { grid: { display: false } }, y: { grid: { color: '#f0f0f0' } } } }
        });

        new Chart(document.getElementById('chartStrengthSingle'), {
            type: 'bar',
            data: { labels: strengthLabels, datasets: [{ label: 'Avg Strength (Day)', data: strengthValues, backgroundColor: strengthValues.map(v => v <= 0 ? '#ef4444' : v < 2 ? '#f59e0b' : '#22c55e'), borderRadius: 6 }] },
            options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false }, tooltip: { callbacks: { label: ctx => `Avg: ${ctx.raw} hari` } } }, scales: { x: { grid: { display: false } }, y: { grid: { color: '#f0f0f0' }, ticks: { callback: v => v + ' hari' } } } }
        });
