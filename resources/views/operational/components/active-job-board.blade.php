            ...
            <div class="lg:col-span-3">
                ...
                <div class="p-4 bg-white border border-slate-200 rounded-3xl min-h-[260px] flex flex-col h-full">
                    ...
                    <div class="mt-auto pt-4 border-t border-slate-200">
                        <div class="grid grid-cols-2 gap-3">
                            <button type="button" id="downtime-btn-{{ $activeJob->id }}" onclick="handleQuickDowntime({{ $activeJob->id }}, 'downtime', 'downtime')" class="dt-btn w-full py-2.5 rounded-xl bg-red-500/10 border border-red-500/30 text-red-400 text-xs font-black uppercase flex items-center justify-center gap-1.5 hover:bg-red-500 hover:text-white transition-all active:translate-y-0.5 group cursor-pointer">Downtime
                                <svg xmlns="http://http://www.w3.org/2000/svg" class="w-3 h-3 shrink-0 opacity-0 -translate-x-1 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-200 fill-current" viewBox="0 0 24 24"><path d="M18 13h-5v5c0 .55-.45 1-1 1s-1-.45-1-1v-5H6c-.55 0-1-.45-1-1s.45-1 1-1h5V6c0-.55.45-1 1-1s1 .45 1 1v5h5c.55 0 1 .45 1 1s-.45 1-1 1z"/></svg>
                            </button>
                            <button type="button" id="tryout-btn-{{ $activeJob->id }}" onclick="handleQuickDowntime({{ $activeJob->id }}, 'tryout', 'try out')" class="to-btn w-full py-2.5 rounded-xl bg-orange-500/10 border border-orange-500/30 text-orange-400 text-xs font-black uppercase flex items-center justify-center gap-1.5 hover:bg-orange-500 hover:text-white transition-all active:translate-y-0.5 group cursor-pointer">Try Out
                                <svg xmlns="http://http://www.w3.org/2000/svg" class="w-3 h-3 shrink-0 opacity-0 -translate-x-1 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-200 fill-current" viewBox="0 0 24 24"><path d="M18 13h-5v5c0 .55-.45 1-1 1s-1-.45-1-1v-5H6c-.55 0-1-.45-1-1s.45-1 1-1h5V6c0-.55.45-1 1-1s1 .45 1 1v5h5c.55 0 1 .45 1 1s-.45 1-1 1z"/></svg>
                            </button>
                            <button type="button" id="break-btn-{{ $activeJob->id }}" onclick="handleQuickDowntime({{ $activeJob->id }}, 'break', 'break time')" class="break-btn col-span-2 w-full py-2.5 rounded-xl bg-slate-500/10 border border-slate-500/30 text-slate-400 text-xs font-black uppercase flex items-center justify-center gap-1.5 hover:bg-slate-500 hover:text-white transition-all active:translate-y-0.5 group cursor-pointer">Break
                                <svg xmlns="http://http://www.w3.org/2000/svg" class="w-3 h-3 shrink-0 opacity-0 -translate-x-1 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-200 fill-current" viewBox="0 0 24 24"><path d="M18 13h-5v5c0 .55-.45 1-1 1s-1-.45-1-1v-5H6c-.55 0-1-.45-1-1s.45-1 1-1h5V6c0-.55.45-1 1-1s1 .45 1 1v5h5c.55 0 1 .45 1 1s-.45 1-1 1z"/></svg>
                            </button>
                        </div>
                    </div>
