    ...
    <div id="active-job-card" class="bg-slate-50 rounded-3xl p-6 shadow-lg shadow-red-900/5 border border-slate-200 text-slate-800 relative overflow-hidden">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-4 relative z-10 items-start">
                ...
                <div class="p-4 bg-white border border-slate-200 rounded-3xl min-h-[260px] flex flex-col h-full">
                    <div class="flex items-center justify-between border-b border-slate-200 pb-3 mb-4">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-[9px] sm:text-[10px] text-emerald-500 font-bold uppercase tracking-wider">Produksi</span>
                        </div>
                    </div>
                    ...
                    <div id="control-board-actions" class="flex flex-col gap-3 mt-3">
                        <button onclick="openDowntimeReport({{ $activeJob->id }}, null)" class="w-full py-3 rounded-xl bg-white border border-slate-300 text-slate-600 font-black text-sm uppercase tracking-wider transition-all flex items-center justify-center gap-2 hover:bg-slate-50 hover:border-slate-400 active:translate-y-0.5 group cursor-pointer">
                    ...
                    <div class="mt-auto pt-4 border-t border-slate-200">
                        <div class="grid grid-cols-2 gap-3">
                            <button type="button" id="downtime-btn-{{ $activeJob->id }}" onclick="handleQuickDowntime({{ $activeJob->id }}, 'downtime', 'downtime')" class="dt-btn relative w-full py-2.5 rounded-xl bg-red-500/10 border border-red-500/30 text-red-400 text-xs font-black uppercase flex items-center justify-center hover:bg-red-500 hover:text-white transition-all active:translate-y-0.5 group cursor-pointer">Downtime
                                <svg xmlns="http://http://www.w3.org/2000/svg" class="absolute right-3 top-1/2 -translate-y-1/2 -translate-x-1 w-3 h-3 opacity-0 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-200 fill-current" viewBox="0 0 24 24"><path d="M18 13h-5v5c0 .55-.45 1-1 1s-1-.45-1-1v-5H6c-.55 0-1-.45-1-1s.45-1 1-1h5V6c0-.55.45-1 1-1s1 .45 1 1v5h5c.55 0 1 .45 1 1s-.45 1-1 1z"/></svg>
                            </button>
                            <button type="button" id="tryout-btn-{{ $activeJob->id }}" onclick="handleQuickDowntime({{ $activeJob->id }}, 'tryout', 'try out')" class="to-btn relative w-full py-2.5 rounded-xl bg-orange-500/10 border border-orange-500/30 text-orange-400 text-xs font-black uppercase flex items-center justify-center hover:bg-orange-500 hover:text-white transition-all active:translate-y-0.5 group cursor-pointer">Try Out
                                <svg xmlns="http://http://www.w3.org/2000/svg" class="absolute right-3 top-1/2 -translate-y-1/2 -translate-x-1 w-3 h-3 opacity-0 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-200 fill-current" viewBox="0 0 24 24"><path d="M18 13h-5v5c0 .55-.45 1-1 1s-1-.45-1-1v-5H6c-.55 0-1-.45-1-1s.45-1 1-1h5V6c0-.55.45-1 1-1s1 .45 1 1v5h5c.55 0 1 .45 1 1s-.45 1-1 1z"/></svg>
                            </button>
                            <button type="button" id="break-btn-{{ $activeJob->id }}" onclick="handleQuickDowntime({{ $activeJob->id }}, 'break', 'break time')" class="break-btn relative col-span-2 w-full py-2.5 rounded-xl bg-slate-500/10 border border-slate-500/30 text-slate-400 text-xs font-black uppercase flex items-center justify-center hover:bg-slate-500 hover:text-white transition-all active:translate-y-0.5 group cursor-pointer">Break
                                <svg xmlns="http://http://www.w3.org/2000/svg" class="absolute right-3 top-1/2 -translate-y-1/2 -translate-x-1 w-3 h-3 opacity-0 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-200 fill-current" viewBox="0 0 24 24"><path d="M18 13h-5v5c0 .55-.45 1-1 1s-1-.45-1-1v-5H6c-.55 0-1-.45-1-1s.45-1 1-1h5V6c0-.55.45-1 1-1s1 .45 1 1v5h5c.55 0 1 .45 1 1s-.45 1-1 1z"/></svg>
                            </button>
                        </div>
                    </div>
            ...
            <div class="lg:col-span-12 mt-4 pt-4 border-t border-slate-200">
                ...
                <div id="rr-list-container-{{ $activeJob->id }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 lg:grid-cols-6 gap-2">
            ...
            <div class="lg:col-span-12 mt-8 pt-8 border-t border-slate-200">
                ...
                <div id="rekam-jejak-grid" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 lg:grid-cols-5 gap-3">
