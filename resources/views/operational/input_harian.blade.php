...
<div class="space-y-6">
        ...
        <div class="flex flex-wrap items-center gap-3">
            ...
            @if(!isset($isHistorical) || !$isHistorical)
                ...
                <div class="flex items-center gap-2">
                    ...
                    <button id="cancelShiftBtn" onclick="cancelShift()"
                        class="flex items-center gap-2 bg-white border border-red-300 hover:bg-red-50 text-red-600 font-bold text-sm px-4 py-2.5 rounded-xl shadow-sm transition-all">
                        <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
                    <input id="shiftCommentInput" type="text" maxlength="255"
                        ...
                        class="w-52 text-sm border border-gray-300 rounded-xl px-3 py-2.5 focus:ring-2 focus:ring-orange-500/30 focus:border-orange-500 outline-none">
    ...
    {{-- REVISI 4: Previous shift leader comment banner (shown on next shift's Input Harian) --}}
    @if(isset($prevShiftComment) && $prevShiftComment)
    <div class="flex items-start gap-3 bg-amber-50 border border-amber-200 rounded-2xl px-5 py-4 mb-6">
        ...
        <div class="min-w-0">
            <p class="text-xs font-bold text-amber-700 uppercase tracking-wide">Catatan Shift Sebelumnya</p>
            <p class="text-sm text-gray-800 mt-1">{{ $prevShiftComment->comment }}</p>
            <p class="text-xs text-gray-500 mt-1">
                {{ $prevShiftComment->submitter?->name ?? 'Leader' }} &bull;
                {{ $prevShiftComment->submitted_at?->format('d M Y H:i') }}
            </p>
        </div>
    </div>
    @endif
...
function cancelShift() {
    showConfirm('Batalkan Akhiri Shift?', 'Data recovery akan dihapus dan shift dibuka kembali. Hanya bisa dilakukan 1x.', function () {
        closeConfirmModal();
        const btn = document.getElementById('cancelShiftBtn');
        btn.disabled = true;
        btn.innerHTML = '<svg class="animate-spin w-4 h-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/></svg> Membatalkan...';

        const params = new URLSearchParams(window.location.search);
        fetch('{{ route('operational.shift.cancel', ['lineId' => '__LINE__']) }}'.replace('__LINE__', params.get('line') || 'Line A'), {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.getAttribute('content') || '',
        ...
        .then(({ status, data }) => {
            if (status === 200 && data.success) {
                showToast(data.message || 'Akhiri Shift dibatalkan.', 'success');
                setTimeout(() => window.location.reload(), 1200);
            } else {
                alert('Gagal: ' + (data.message || 'Unknown error'));
                btn.disabled = false;
                btn.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M6 18L18 6M6 6l12 12"/></svg> Batalkan';
            }
        })
        .catch(err => {
            alert('Error: ' + err.message);
