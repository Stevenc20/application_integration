...
<head>
    <meta charset="utf-8">
    <title>Daftar Material PDF</title>
    <style>
        body {
...
<body>
    ...
    <div class="meta-info" style="border-bottom: 1px dashed #cbd5e1; padding-bottom: 4px; margin-bottom: 8px;">
        <strong>Filter aktif:</strong> {{ $filterStr }} &nbsp;|&nbsp; <strong>Total:</strong> {{ $materials->count() }} material
    </div>
    ...
    <table class="table-data">
        <thead>
            <tr>
                ...
                <th style="width: 15%; text-align: right;">TOTAL MIN STOK</th>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="code-td">{{ $material->kode }}</td>
                <td style="font-weight: bold;">
                    {{ $material->nama }}
                    @if($material->stok < $material->min_stok)
                        <span class="warn-text">  Minim</span>
                    @endif
                </td>
                <td>{{ $material->tipe }}</td>
                <td>{{ $material->uom }}</td>
                <td style="text-align: right; font-weight: bold;">
                    {{ number_format($material->stok, 3, '.', ',') }}
                </td>
                <td style="text-align: right;">
                    {{ number_format($material->min_stok, 3, '.', ',') }}
                </td>
                <td style="text-align: center; font-weight: bold; color: {{ $material->status === 'Aktif' ? '#15803d' : '#b91c1c' }};">
                    {{ $material->status }}
                </td>
            </tr>
            @endforeach
    ...
    <div class="footer">
        <table class="meta-flex">
            <tr>
                <td>
                    Total {{ $materials->count() }} material ditampilkan.
                </td>
                <td style="text-align: right;">
