(function() {
    ...
    function buildColsFormula(total, tactTime, ctDimensi, ctTanpaDimensi, mode = 'auto', existingCols = []) {
        const keepers = existingCols.filter(c => typeof c === 'string' && c.toUpperCase().startsWith('KEEPER'));
        const n = parseInt(total) || 0;

        if (n <= 0) return keepers;
        const calc = calcSamplingIntervals(tactTime, ctDimensi, ctTanpaDimensi, mode);
        if (!calc) {
            const baseCols = buildCols(n);
            return [...baseCols, ...keepers];
        }
        ...
        const resCols = [...new Set(cols)].sort((a, b) => a - b);
        return [...resCols, ...keepers];
    }


    window.itemCheckForm = function(config = {}) {
        return {
            // ==========================================
            // STATE & INITIALIZATION
            // ==========================================
            editId: config.editId || null,
            previewData: config.previewData || null,
            role: config.role || 'Guest',
            userName: config.userName || '',
            userId: config.userId || '',
            saving: false, savingDraft: false, loadingData: !!config.editId || !!config.previewData,
            activeTab: 'main',
            toast: null,
            ...
            tandemUnfinishedId: config.tandemUnfinishedId || null,
            ...
            waktuMulai: null, waktuSelesai: null,
            showPauseModal: false, pauseReason: '', customPauseReason: '',
            ...
            masterFmSig: null, masterFmName: '', masterGlSig: null, masterGlName: '',
            ...
            svgCanvasH: 580,
            svgToolConfig: [
                { id: 'select', icon: 'ÃƒÂ¢Ã‚Â¬Ã¢â‚¬Â ', label: 'Pilih' },
                { id: 'rect',   icon: 'ÃƒÂ¢Ã‚Â¬Ã…â€œ', label: 'Kotak' },
                { id: 'circle', icon: 'ÃƒÂ¢Ã‚Â­Ã¢â‚¬Â¢', label: 'Lingkaran' },
                { id: 'zone',   icon: 'ÃƒÂ°Ã…Â¸Ã¢â‚¬ÂÃ‚Â²', label: 'Zona' },
                { id: 'text',   icon: 'T',  label: 'Teks' },
                { id: 'line',   icon: 'ÃƒÂ¢Ã¢â‚¬Â¢Ã‚Â±',  label: 'Garis' },
                { id: 'arrow',  icon: '→',  label: 'Panah' },
                { id: 'delete', icon: 'ÃƒÂ°Ã…Â¸Ã¢â‚¬â€Ã¢â‚¬Ëœ', label: 'Hapus' },
            ],
            ...
            ngCurrentPhoto: '',
            ngCurrentQty: 1,
            ngCurrentQtyRepair: 0,
            ngCurrentQtyReject: 0,
            ngOtherProblemText: '',
            ngDetails: {},
...
bundleChecks: [], bundleTindakan: '',
            ...
            // Signature PIN Modal State
            showPinModal: false,
            pinInput: '',
            verifyingPin: false,
            activePinType: '',
            activePinUserId: null,
            masterUserId: null,
            masterTargetType: null,
            ...
            assignedGlId: '', assignedForemanId: '', assignedOperatorId: '', operatorName: '',
            ...
            quickRevPresets: [
                { key: 'item_check', icon: `<svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 002 2h2a2 2 0 002-2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01"/></svg>`, label: 'Tabel Inspeksi' },
            ...
            get canLeaderEditStandard() {
                ...
                return [
                    ...
                    'ready_for_qc', 'locked', 'waiting_qc_approval',
                ].includes(this.status);
            },
            get canLeaderEditSampling() { return this.canLeaderEditStandard; },
            /** Setelah operator submit → setiap edit bagian atas wajib catat Revision Record */
            ...
            get isQCSectionOpen() { 
                return ['admin', 'operator', 'group leader', 'foreman'].includes((this.role||'').toLowerCase()) && 
                       ['in_progress', 'draft', 'revision', 'waiting_gl', 'waiting_foreman', 'waiting_qc_approval', 'ready_for_qc', 'locked', 'approved', 'finished'].includes(this.status); 
            },
            /** LI selesai / bukan operator QC → modal NG hanya lihat, tidak edit, KECUALI KEEPER */
            get isNgModalReadOnly() {
                if (String(this.ngTargetCol || '').toUpperCase().startsWith('KEEPER') && this.isOperator) {
                    return false;
                }
                return ['finished', 'approved'].includes(this.status)
                    || !(this.isOperator && this.isQCSectionOpen);
            },
            /** Sel masih NG (tanda âœ—) atau jumlah hole tidak sesuai standar */
            ...
            pruneStaleNgDetails() {
                ...
                Object.keys(this.ngDetails || {}).forEach((key) => {
                    ...
                    const col = parts[1];
                    if (Number.isNaN(row) || !col || !this.isNgCellActive(row, col)) {
            ...
            async init() {
                ...
                // Auto-resume timer if document was paused due to QC submission / rejection
                if (this.editId && this.isOperator && this.waktuSelesai && ['in_progress', 'revision'].includes(this.status)) {
                    try {
                        const res = await apiFetch(`/api/item-check/${this.editId}/resume`, { method: 'POST' });
                        if (res.waktu_mulai) {
                            this.waktuMulai = res.waktu_mulai;
                            this.waktuSelesai = null;
                            console.log('[TIMER] Timer automatically resumed from server idle time.');
                        }
                    } catch (e) {
                ...
                // Restore pause state
                const pauseData = localStorage.getItem(`li_pause_${this.editId}`);
                if (pauseData) {
                    try {
                        const parsed = JSON.parse(pauseData);
                        this.isPaused = parsed.isPaused || false;
                        this.pauseStartTime = parsed.pauseStartTime || null;
                        this.totalPausedMs = parsed.totalPausedMs || 0;
                        this.pauseReason = parsed.pauseReason || '';
                        this.customPauseReason = parsed.customPauseReason || '';
                    } catch(e) {}
                }

                if (this.previewData) {
                    await this.loadDataFromObject(this.previewData);
                ...
                // Normalisasi dan Auto-Detect Shift
                this.shift = this.normalizeShift(this.shift);
                if (!this.shift) {
                    const hour = new Date().getHours();
                    this.shift = (hour >= 6 && hour < 18) ? 'Shift Pagi' : 'Shift Malam';
                }
            ...
            normalizeShift(val) {
                ...
                if (s.includes('pagi') || s === 'shift 1' || s === '1') return 'Shift Pagi';
                if (s.includes('malam') || s.includes('sore') || s === 'shift 2' || s === 'shift 3' || s === '2' || s === '3') return 'Shift Malam';
                return val;
            },
            ...
            async fetchUsers() {
                try {
                    ...
                    this.glUsers = list.filter(u => u.role === 'Group Leader').map(u => ({ id: u.id, nama: u.name, phone: u.phone || '', empNo: u.employee_id || '' }));
                    ...
                    )).map(u => ({ id: u.id, nama: u.name, phone: u.phone || '', empNo: u.employee_id || '' }));
            ...
            saveDraft() {
                try {
                    ...
                    const d = { 
                        ...
                        ngCurrentPhoto: this.ngCurrentPhoto,
                        ngCurrentQty: this.ngCurrentQty,
            ...
            rebuildCols() {
                if (this.customSamplingCols && this.customSamplingCols.length > 0) {
                    ...
                    // Jika totalPcs lebih besar dari max sampling kolom yang tersimpan, kita extend sisanya?
                    // Atau kita tetapkan target terakhir adalah totalPcs
                    if (this.totalPcs > 0) {
                ...
                return buildColsFormula(
                    ...
                    this.cols // Pass existing cols to preserve keeper columns
                );
            },
            
            get waktuStandarText() {
                if (!this.tactTime && !this.ctDimensi && !this.ctTanpaDimensi) return "-";
                const numSamples = this.cols.filter(c => typeof c === 'number').length;
                const ctDim = parseFloat(this.ctDimensi) || 0;
                const ctVis = parseFloat(this.ctTanpaDimensi) || 0;
                
                // Tentukan apakah part ini memiliki spesifikasi dimensi
                const hasDimensi = this.dimStd && this.dimStd.some(d => d.item && d.item.trim() !== '');
                
                // Gunakan CT yang sesuai per sampling
                const secondsPerSample = hasDimensi ? ctDim : ctVis;
                
                if (secondsPerSample <= 0) return "-";
                
                // Total Waktu = Jumlah Sampling x CT Per Sampling
                const totalSeconds = numSamples * secondsPerSample;
                const minutes = Math.floor(totalSeconds / 60);
                
                if (minutes === 0 && totalSeconds > 0) return `${Math.floor(totalSeconds)} Detik`;
                return `${minutes} Menit`;
            },

            getEffectiveElapsedMs() {
                if (!this.waktuMulai) return 0;
                const end = this.waktuSelesai ? new Date(this.waktuSelesai).getTime() : this.currentTime;
                const start = new Date(this.waktuMulai).getTime();
                let diffMs = end - start;
                
                if (this.totalPausedMs) diffMs -= this.totalPausedMs;
                if (this.isPaused && this.pauseStartTime && !this.waktuSelesai) {
                    diffMs -= (end - new Date(this.pauseStartTime).getTime());
                }
            ...
            togglePause() {
                ...
                if (this.isPaused) {
                    ...
                    if (this.pauseStartTime) {
                        pausedDurationMs = (new Date().getTime() - new Date(this.pauseStartTime).getTime());
                        this.totalPausedMs += pausedDurationMs;
                    }
                    
                    // Catat ke Remarks jika ada alasan jeda
                    if (this.pauseReason && pausedDurationMs > 0) {
                        let minutes = Math.floor(pausedDurationMs / 60000);
                        if (minutes < 1) minutes = '< 1';
                        
                        let reasonText = this.pauseReason;
                        if (reasonText === 'Lainnya') reasonText = this.customPauseReason || 'Lainnya';
                        
                        let logMsg = `[SISTEM]: Timer dijeda selama ${minutes} Menit (Alasan: ${reasonText}).`;
                        if (this.catatan && String(this.catatan).trim().length > 0) {
                            this.catatan = String(this.catatan).trim() + '\n' + logMsg;
                        } else {
                            this.catatan = logMsg;
                        }
                        
                        this.showToast('success', 'Info jeda otomatis ditambahkan ke catatan.');
                    }
                    ...
                    localStorage.setItem(`li_pause_${this.editId}`, JSON.stringify({
                        isPaused: this.isPaused,
                        pauseStartTime: this.pauseStartTime,
                        totalPausedMs: this.totalPausedMs,
                        pauseReason: this.pauseReason,
                        customPauseReason: this.customPauseReason
                    }));
                } else {
                    // Jangan langsung pause, tampilkan modal pilihan alasan
            ...
            confirmPause() {
                if (!this.pauseReason) {
                    this.showToast('error', 'Silakan pilih alasan jeda terlebih dahulu!');
                    return;
                }
                if (this.pauseReason === 'Lainnya' && !(this.customPauseReason || '').trim()) {
                    this.showToast('error', 'Silakan tuliskan alasan Anda secara spesifik!');
                    return;
                }
            ...
            get waktuAktualText() {
                if (!this.waktuMulai) return "Belum dimulai";
                const diffMs = this.getEffectiveElapsedMs();
                if (diffMs < 0) return "-";
            ...
            get liveTimerDisplay() {
                if (!this.waktuMulai) return "00:00:00";
                const diffMs = this.getEffectiveElapsedMs();
            ...
            get samplingProgressData() {
                if (!this.waktuMulai) return null;
                const diffSecs = Math.floor(this.getEffectiveElapsedMs() / 1000);
                
                const hasDimensi = this.dimStd && this.dimStd.some(d => d.item && d.item.trim() !== '');
                const secondsPerSample = hasDimensi ? (parseFloat(this.ctDimensi) || 0) : (parseFloat(this.ctTanpaDimensi) || 0);
                const sampleCols = this.cols.filter(c => typeof c === 'number');
                
                if (secondsPerSample <= 0 || sampleCols.length === 0) return null;
                
                const currentSampleIndex = Math.floor(diffSecs / secondsPerSample);
                const isOverdue = currentSampleIndex >= sampleCols.length;
                const activeIndex = isOverdue ? sampleCols.length - 1 : currentSampleIndex;
                
                const currentKolom = sampleCols[activeIndex];
                const elapsedInSample = diffSecs % secondsPerSample;
                let remainingInSample = secondsPerSample - elapsedInSample;
                
                if (isOverdue) remainingInSample = 0;
                
                const m = Math.floor(remainingInSample / 60).toString().padStart(2, '0');
                const s = Math.floor(remainingInSample % 60).toString().padStart(2, '0');
                
                const progressPercent = isOverdue ? 100 : Math.min(100, Math.max(0, (elapsedInSample / secondsPerSample) * 100));
                
                let totalOverdueSecs = 0;
                if (isOverdue) totalOverdueSecs = diffSecs - (sampleCols.length * secondsPerSample);
                
                const ovM = Math.floor(totalOverdueSecs / 60).toString().padStart(2, '0');
                const ovS = Math.floor(totalOverdueSecs % 60).toString().padStart(2, '0');
                
                return {
                    kolom: currentKolom,
                    sampelNo: activeIndex + 1,
                    remainingText: isOverdue ? `+ ${ovM}:${ovS}` : `${m}:${s}`,
            ...
            addKeeperCheckCol() {
                const keepers = this.cols.filter(c => typeof c === 'string' && c.toUpperCase().startsWith('KEEPER'));
                const nextNum = keepers.length + 1;
                this.cols.push(`KEEPER ${nextNum}`);
                this.showToast('success', `Kolom Temuan Keeper ${nextNum} berhasil ditambahkan.`);
            },

            removeKeeperCol(colName) {
                if (!confirm(`Hapus kolom ${colName} secara permanen? Data yang ada di kolom ini akan hilang.`)) return;
                
                // Hapus dari list kolom
                this.cols = this.cols.filter(c => c !== colName);
                
                // Hapus data dari appData (Appearance)
                ...
                // Hapus data dari dimData (Dimensi)
                Object.keys(this.dimData).forEach(key => {
                    if (key.endsWith(`_${colName}`)) {
                        delete this.dimData[key];
                    }
                });
                
                // Hapus NG Details
                if (this.ngDetails && typeof this.ngDetails === 'object') {
                    Object.keys(this.ngDetails).forEach(key => {
                        if (key.endsWith(`_${colName}`)) {
                            delete this.ngDetails[key];
                        }
                ...
                this.showToast('success', `Kolom ${colName} berhasil dihapus.`);
            ...
            get samplingPreview() {
                ...
                return `${c.modeLabel}: +${c.interval1} lalu +${c.interval2} → ${this.cols.length} kolom`;
            },
            ...
            restoreDraft() {
                try {
                    ...
                    Object.keys(d).forEach(k => { 
                        if (d[k] !== undefined) {
                            if (k === 'shift') this[k] = this.normalizeShift(d[k]);
                            else this[k] = d[k]; 
                        }
                    });
                    this.pruneStaleNgDetails();

                    const keeperColsSet = new Set();
                    Object.keys(this.dimData).forEach(k => {
                        const col = String(k).split('_')[1];
                        if (col && col.toUpperCase().startsWith('KEEPER')) keeperColsSet.add(col);
                    });
                    Object.keys(this.appData).forEach(k => {
                        const col = String(k).split('_')[1];
                        if (col && col.toUpperCase().startsWith('KEEPER')) keeperColsSet.add(col);
                    });
                    Array.from(keeperColsSet).forEach(c => {
                        if (!this.cols.includes(c)) this.cols.push(c);
                    });
            ...
            setAppVal(row, col, val) {
                ...
                if (current === val) {
                    ...
                    this.appData = { ...this.appData }; // Force Alpine.js reactivity
            ...
            openNgReasonModal(row, col) {
                ...
                this.ngCurrentPhoto = existing.photo || '';
                this.ngCurrentDisposition = existing.disposisi || '';
                this.ngCurrentQty = existing.qty || 1;
                this.ngCurrentQtyRepair = existing.qty_repair !== undefined ? existing.qty_repair : (existing.disposisi === 'repair' ? existing.qty : 0);
                this.ngCurrentQtyReject = existing.qty_reject !== undefined ? existing.qty_reject : (existing.disposisi === 'reject' ? existing.qty : 0);
                
                // Reset & Populate Problems based on context
                const problems = {};
                this.ngOtherProblemText = '';
                const defaultProbs = this.getNgProblemsList(row).filter(x => x !== 'LAINNYA');
                
                this.getNgProblemsList(row).forEach(p => {
                    if (p === 'LAINNYA') {
                        const customProb = (existing.problems || []).find(ep => !defaultProbs.includes(ep));
                        if (customProb) {
                            problems['LAINNYA'] = true;
                            this.ngOtherProblemText = customProb;
                        } else {
                            problems['LAINNYA'] = false;
                        }
                    } else {
            ...
            getNgDetailsForSave() {
                if (typeof this.pruneStaleNgDetails === 'function') {
                    this.pruneStaleNgDetails();
                }
                ...
                Object.keys(raw).forEach(k => {
                    ...
                    const row = parseInt(parts[0], 10);
                    const col = parts[1]; // Do NOT parseInt the col because KEEPER cols are strings like 'KEEPER 1'
                    if (!Number.isNaN(row) && col && this.isNgCellActive(row, col)) {
                        out[k] = raw[k];
                    }
                });
            ...
            async persistNgDetails() {
                ...
                try {
                    await apiFetch(`/api/item-check/${this.editId}`, {
                        method: 'PUT',
                        body: JSON.stringify({
            ...
            // ÃƒÂ¢Ã¢â‚¬Â Ã¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬Â Ã¢â€šÂ¬ Bundle Check Prompt (dipanggil dari tab Standard) ÃƒÂ¢Ã¢â‚¬Â Ã¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬Â Ã¢â€šÂ¬
            requestBundleCheck() {
            ...
            saveNgReason() {
                ...
                this.ngDetails[`${this.ngTargetRow}_${this.ngTargetCol}`] = {
                    proses: isHole ? '' : Object.keys(this.ngCurrentProcesses).filter(k => this.ngCurrentProcesses[k]).join(', '),
                    areas: isHole ? [] : Object.keys(this.ngCurrentAreas).filter(k => this.ngCurrentAreas[k]).map(k => parseInt(k)),
                    problems: isHole ? ['JUMLAH HOLE TIDAK SESUAI'] : (() => {
                        let probs = Object.keys(this.ngCurrentProblems).filter(k => this.ngCurrentProblems[k] && k !== 'LAINNYA');
                        if (this.ngCurrentProblems['LAINNYA'] && this.ngOtherProblemText.trim()) {
                            probs.push(this.ngOtherProblemText.trim().toUpperCase());
                        }
                        return probs;
                    })(),
                    ...
                    photo: this.ngCurrentPhoto,
                    qty: String(this.ngTargetCol).toUpperCase().startsWith('KEEPER') ? ((parseInt(this.ngCurrentQtyRepair) || 0) + (parseInt(this.ngCurrentQtyReject) || 0)) || 1 : 1,
                    qty_repair: String(this.ngTargetCol).toUpperCase().startsWith('KEEPER') ? (parseInt(this.ngCurrentQtyRepair) || 0) : (this.ngCurrentDisposition === 'repair' ? 1 : 0),
                    qty_reject: String(this.ngTargetCol).toUpperCase().startsWith('KEEPER') ? (parseInt(this.ngCurrentQtyReject) || 0) : (this.ngCurrentDisposition === 'reject' ? 1 : 0),
                    disposisi: isHole ? '' : (String(this.ngTargetCol).toUpperCase().startsWith('KEEPER') ? 'multi' : this.ngCurrentDisposition),
            ...
            async handleNgPhotoUpload(e) {
                const file = e.target.files[0];
                if (!file) return;
                try {
                    this.showToast('info', 'Mengunggah foto...');
                    const fd = new FormData();
                    fd.append('file', file);
                    const res = await window.axios.post('/api/upload-sketch', fd, {
                        headers: { 'Content-Type': 'multipart/form-data' }
                    });
                    this.ngCurrentPhoto = res.data.url;
                    this.showToast('success', 'Foto berhasil diunggah!');
                } catch(err) {
                    this.showToast('error', 'Gagal mengunggah foto!');
                    console.error(err);
                }
            },
            ...
            updateRepairRejectCounts() {
                ...
                Object.values(this.ngDetails).forEach(detail => {
                    if (detail) {
                        if (detail.qty_repair !== undefined || detail.qty_reject !== undefined) {
                            repairCount += parseInt(detail.qty_repair) || 0;
                            rejectCount += parseInt(detail.qty_reject) || 0;
                        } else {
                            const qty = parseInt(detail.qty) || 1;
                            if (detail.disposisi === 'repair') {
                                repairCount += qty;
                            } else if (detail.disposisi === 'reject') {
                                rejectCount += qty;
                            }
                        }
                    }
            ...
            isColComplete(col) {
                ...
                // UPDATE: Dimensi HANYA diwajibkan untuk pengecekan awal (kolom pertama)
                if (col === this.cols[0]) {
            ...
            isColUnlocked(col) {
                if (['finished', 'approved', 'locked'].includes(this.status)) return false;
                
                if (typeof col === 'string' && col.toUpperCase().startsWith('KEEPER')) return true;
                
                const normalCols = this.cols.filter(c => !(typeof c === 'string' && c.toUpperCase().startsWith('KEEPER')));
                const index = normalCols.indexOf(col);
                if (index <= 0) return true;
                return this.isColComplete(normalCols[index - 1]);
            },
            ...
            formatDimStdBrief(d) {
                if (!d || (!d.item && !d.nominal && !d.label)) return '- ';
            ...
            saveAppStandardModal() {
                ...
                const before = (this.appItems[ri] || '').trim() || '- ';
                ...
                const after = (this.appItems[ri] || '').trim() || '- ';
                if (this.shouldLogLeaderRevision && before !== after) {
            ...
            setColDimNominal(col) {
                ...
                this.dimData = { ...this.dimData }; // Force Alpine.js reactivity
            },
            ...
            computedOkCount() {
                let okCount = 0;
                // iterate over this.cols, if getColJudgement(col) === 'OK', then okCount++
            ...
            cycleBundleApp(bundle, s, idx) {
                ...
                bundle.samples = { ...bundle.samples }; // Force Alpine.js reactivity
            },
            ...
            sendWhatsAppNotification(roleType) {
                ...
                const partInfo = this.partName || '- ';
                const jobInfo = this.jobNo || '- ';
                const roleLabel = roleType === 'gl' ? 'GL' : 'Foreman';

                let message = '';
                if (severity === 'critical_ng') {
                    message = `ÃƒÂ°Ã…Â¸Ã…Â¡Ã‚Â¨ *[CRITICAL ALERT - SEGERA KE JALUR]* ÃƒÂ°Ã…Â¸Ã…Â¡Ã‚Â¨

Halo Pak/Bu ${user.nama}, terdeteksi temuan *NG KRITIS* di Press Line!

ÃƒÂ°Ã…Â¸Ã¢â‚¬Å“Ã¢â‚¬Â¹ *Detail Dokumen:*
- Part Name: ${partInfo}
- Job No: ${jobInfo}
...
ÃƒÂ¢Ã…Â¡Ã‚Â ÃƒÂ¯Ã‚Â¸Ã‚Â  Mohon *SEGERA TURUN KE JALUR* untuk melakukan investigasi fisik langsung bersama GL dan Operator sebelum menyetujui dokumen ini.

ÃƒÂ°Ã…Â¸Ã¢â‚¬Â Ã¢â‚¬â€  Link Dokumen: ${docUrl}`;
                } else {
                    message = `ÃƒÂ°Ã…Â¸Ã¢â‚¬Å“Ã¢â‚¬Â¹ *Permintaan Verifikasi Lembar Inspeksi*
...
ÃƒÂ°Ã…Â¸Ã¢â‚¬Â Ã¢â‚¬â€  Klik link berikut untuk membuka & menyetujui dokumen:
${docUrl}`;
                }
                ...
                if (phone) {
                    ...
                    // Tidak ada nomor telepon -  buka dialog input
                ...
                try {
                    ...
                    if (res && res.success && res.data) {
                        ...
                        if (this.intercomCall.status === 'arrived') {
                            // GL sudah check-in fisik → tutup modal, tampilkan banner hijau
                            this.stopRingtone();
                            ...
                            // GL dalam perjalanan → JANGAN tutup modal (GL perlu klik check-in)
                            this.stopRingtone();
                            clearInterval(this.intercomInterval);
                            this.intercomInterval = null;
                            if (!this._answeredToastShown) {
                                this._answeredToastShown = true;
                                this.showToast('success', `ÃƒÂ°Ã…Â¸Ã‚Â Ã†â€™ ${this.intercomCall.responder_name || 'GL/Foreman'} sedang dalam perjalanan!`);
                            }
            ...
            signRemotely(roleType) {
                ...
                ctx.fillText('âœ“ TTD MANDIRI SAH -  ' + new Date().toLocaleString('id-ID'), 200, 100);

                const sigData = canvas.toDataURL('image/png');

                if (roleType === 'gl') {
                    this.glParaf = sigData;
                    this.showToast('success', 'âœ“ TTD Mandiri GL berhasil disimpan!');
                } else if (roleType === 'foreman') {
                    this.fmParaf = sigData;
                    this.showToast('success', 'âœ“ TTD Mandiri Foreman berhasil disimpan!');
                } else if (roleType === 'gl_bundle') {
                    this.bundleGlSig = sigData;
                    this.showToast('success', 'âœ“ TTD Mandiri GL (Bundle) berhasil disimpan!');
                } else if (roleType === 'fm_bundle') {
                    this.bundleFmSig = sigData;
                    this.showToast('success', 'âœ“ TTD Mandiri Foreman (Bundle) berhasil disimpan!');
                }
            },
            
            async autoSignOperator(type) {
                const targetUserId = this.assignedOperatorId;
                if (!targetUserId) {
                    this.showToast('error', 'Silakan pilih Operator dari dropdown terlebih dahulu.');
                    return;
                }
                
                try {
                    const res = await fetch(`/api/signature/get-signature?user_id=${targetUserId}`, {
                        method: 'GET',
                        headers: {
                    ...
                    if (data.success && data.signature) {
                        if (type === 'operator_ttd') {
                            this.bundlePrepSig = data.signature;
                            this.showToast('success', 'TTD Operator berhasil disematkan!');
                            ...
                            if (this.fieldRevisions && this.fieldRevisions['item_check']) {
                                ...
                                this.showToast('info', 'Catatan revisi ditandai selesai secara otomatis.');
                            }
                        }
                    } else {
                        // Belum punya TTD di database, minta buat Master TTD!
                        this.showToast('info', 'Anda belum memiliki tanda tangan. Silakan buat TTD Anda sekarang.');
                        this.masterUserId = targetUserId;
                        this.masterTargetType = type;
                        this.openSignaturePad('master_signature');
                    }
                } catch (e) {
            ...
            initiateUpdateMaster(type, userId) {
                if (confirm('Anda yakin ingin memperbarui/mengganti Master TTD untuk akun ini?')) {
                    if (type === 'operator_ttd' || type === 'prep') {
                        // Operator langsung buka pad
                        this.masterUserId = userId;
                        this.masterTargetType = type;
                        this.openSignaturePad('master_signature');
                    } else {
                        // GL/Foreman wajib PIN ulang demi keamanan
                        this.openPinPad('update_master_' + type, userId);
                    }
                }
            },

            async openPinPad(type, userId) {
                if (!userId) {
                    this.showToast('error', "Silakan pilih User terlebih dahulu dari dropdown.");
                    return;
                }
                
                // Jika user yang dituju adalah user yang sedang login, langsung ambil TTD tanpa PIN
                if (this.userId && this.userId == userId) {
                    this.saving = true;
                    try {
                        const sigRes = await fetch(`/api/signature/get-signature?user_id=${userId}`, {
                            method: 'GET',
                        ...
                        if (sigRes.ok) {
                            ...
                            if (sigData.signature) {
                                this.processSignature(type, sigData.signature);
                            } else {
                                this.showToast('error', 'Anda belum mengatur Master Tanda Tangan');
                                this.masterUserId = userId;
                                this.masterTargetType = type;
                                this.openSignaturePad('master_signature');
                            }
                        } else {
                            this.showToast('error', 'Gagal memuat tanda tangan Anda.');
                        }
                    } catch(e) {
            ...
            processSignature(type, signature) {
                if (type === 'prep') {
                    this.prepSig = signature;
                    if (this.tandem_check_id) this.bundlePrepSig = signature;
                }
                else if (type === 'operator_ttd') {
                    this.bundlePrepSig = signature;
                }
                else if (type === 'fm') this.glSig = signature;
                else if (type === 'spv') this.fmSig = signature;
                else if (type === 'gl_global' || type === 'glParaf') {
                    this.bundleGlSig = signature;
                }
                else if (type === 'fm_global' || type === 'foremanParaf' || type === 'fmParaf') {
                    this.bundleFmSig = signature;
                }
            },
            async submitPinSignature() {
                if (this.pinInput.length !== 6 || !this.activePinUserId) return;
                
                this.verifyingPin = true;
                try {
                    const res = await fetch('/api/signature/verify-pin', {
                        method: 'POST',
                        headers: {
                    ...
                    if (res.ok && data.success) {
                        ...
                        if (this.activePinType.startsWith('update_master_')) {
                            const originalType = this.activePinType.replace('update_master_', '');
                            this.masterUserId = this.activePinUserId;
                            this.masterTargetType = originalType;
                            this.closePinModal();
                            this.openSignaturePad('master_signature');
                            return;
                        }
                        ...
                        if (sigData) {
                            this.processSignature(this.activePinType, sigData);
                            this.showToast('success', `Berhasil diautentikasi sebagai ${data.user_name}`);
                        } else {
                            this.showToast('info', 'Tanda tangan belum ada. Silakan buat Master TTD Anda.');
                            this.masterUserId = this.activePinUserId;
                            this.masterTargetType = this.activePinType;
                            this.openSignaturePad('master_signature');
                        }
                        this.closePinModal();
                    } else {
                        this.showToast('error', data.message || 'PIN salah atau tidak valid.');
                        this.pinInput = '';
                    }
                } catch (e) {
             ...
             saveSignature() {
                 ...
                 if (this.activePad === 'prep') {
                 ...
                 else if (this.activePad === 'fm_global' || this.activePad === 'fmParaf' || this.activePad === 'foremanParaf') this.bundleFmSig = data;
                 else if (this.activePad === 'master_signature') {
                     // Simpan ke database
                     try {
                         apiFetch('/api/signature/save-master', {
                             method: 'POST',
                             body: JSON.stringify({
                             ...
                             this.showToast('success', 'Tanda tangan berhasil disimpan sebagai Master!');
                         }).catch(err => {
                             console.error('Error saving master signature:', err);
                         });
                     } catch(e) {}
                     
                     // Apply ke field yang bersangkutan
                     if (this.masterTargetType === 'operator_ttd' || this.masterTargetType === 'prep') {
                         this.bundlePrepSig = data;
                         // Auto-resolve revisi item check jika sebelumnya ditolak
                         if (this.fieldRevisions && this.fieldRevisions['item_check']) {
                             this.fieldRevisions['item_check'].resolved = true;
                     ...
                     else if (this.masterTargetType === 'fm') this.glSig = data;
                     else if (this.masterTargetType === 'spv') this.fmSig = data;
                     else if (this.masterTargetType === 'gl_global' || this.masterTargetType === 'glParaf') this.bundleGlSig = data;
                     else if (this.masterTargetType === 'fm_global' || this.masterTargetType === 'fmParaf' || this.masterTargetType === 'foremanParaf') this.bundleFmSig = data;
                 }

                 this.closeSignaturePad();
                 this.showToast('success', 'TTD Berhasil disematkan');
             },

            // ==========================================
            // DATA LOADING & SAVING (API)
            // ==========================================
            async pullProductionStats() {
                if (!this.jobNo) return;
                try {
                    const res = await apiFetch(`/api/production-schedules/${encodeURIComponent(this.jobNo)}`);
                    if (res && res.actual_qty !== undefined) {
                        this.totalPcs = res.actual_qty;
                        this.reject = res.ng_qty;
                        this.repair = res.repair_qty;
                        if (res.tanggal) this.tanggal = String(res.tanggal).slice(0, 10);
                        
                        // Generate preview cols based on current values immediately if in preview mode
                        if (this.previewData) {
                            this.cols = this.rebuildCols();
                        }
                    }
                } catch (e) {
                    console.log('Gagal menarik data produksi otomatis:', e);
                }
            },
            ...
            async loadData() {
                this.loadingData = true;
                try {
                    const res = await apiFetch(`/api/item-check/${this.editId}`);
                    const d = res.data ?? res;
                    await this.loadDataFromObject(d);
                } catch (e) {
                    this.showToast('error', 'Gagal memuat data');
            ...
            async loadDataFromObject(d) {
                this.jobNo = d.job_no || ''; this.partName = d.part_name || '';
                    ...
                    this.shift = this.normalizeShift(d.shift || ''); 
                    this.totalPcs = d.total_pcs !== undefined ? d.total_pcs : (d.total_produksi || 0); this.repair = d.repair || 0; this.reject = d.reject || 0;
                    
                    this.qprId = d.qpr_id || null;
                    this.qprGenerated = d.qpr_generated || false;
                    
                    // Status management
                    this.status = d.status || 'draft';
                    this.waktuMulai = d.waktu_mulai || null;
                    this.waktuSelesai = d.waktu_selesai || null;
                    ...
                    if (d.qg_name) {
                        this.qgName = d.qg_name.replace(' (Auto)', '');
                    }
                    this.judgement = d.qg_judgement || '';
                    // #region agent log -  H-D: [] dari DB membuat ngDetails array → hilang saat JSON.stringify
                    ...
                    this.glSig = null; // Unused in ItemCheck
                    this.fmSig = null; // Unused in ItemCheck
                    this.masterGlSig = formatSigPath(d.master_paraf_gl);
                    this.masterFmSig = formatSigPath(d.master_paraf_foreman);
                    this.masterGlName = (d.master_gl_name || '').replace(' (Auto)', '');
                    this.masterFmName = (d.master_frm_name || '').replace(' (Auto)', '');
                    ...
                    this.bundleGlSig = formatSigPath(d.paraf_gl) || formatSigPath(d.paraf_gl_bottom) || null;
                    this.bundleFmSig = formatSigPath(d.paraf_foreman) || formatSigPath(d.paraf_foreman_bottom) || null;
                    this.bundleGlName = (d.paraf_gl_bottom_name || '').replace(' (Auto)', ''); 
                    this.bundleFmName = (d.paraf_fm_bottom_name || '').replace(' (Auto)', '');
                    ...
                    this.operatorName = d.operator_name || '';
                    this.operatorClaimedAt = d.operator_claimed_at || null;
                    ...
                    // Load dimData and appData directly if returned by API
                    if (d.dimData && Object.keys(d.dimData).length > 0) {
                        this.dimData = typeof d.dimData === 'string' ? JSON.parse(d.dimData) : Object.assign({}, d.dimData);
                    }
                    if (d.appData && Object.keys(d.appData).length > 0) {
                        this.appData = typeof d.appData === 'string' ? JSON.parse(d.appData) : Object.assign({}, d.appData);
                    }
                    ...
                    for (let i = 0; i < 7; i++) {
                        ...
                        // Load from generic JSON structure if exists AND native dimData wasn't loaded
                        if (!d.dimData || Object.keys(d.dimData).length === 0) {
                    ...
                    for (let r = 0; r < 9; r++) {
                        if (!d.appData || Object.keys(d.appData).length === 0) {
                            ...
                            Object.entries(results).forEach(([c, v]) => {
                                this.appData[`${r}_${c}`] = (v === 'âœ“' || v === 'ok') ? 'ok' : (v === 'âœ—' || v === 'ng') ? 'ng' : v;
                            });
                        }
            ...
            buildPayload(targetStatus) {
                // Ensure bundleGlName and bundleFmName are synced with assigned IDs
                if (this.assignedGlId) {
                    const selGl = this.glUsers.find(u => String(u.id) === String(this.assignedGlId));
                    if (selGl) this.bundleGlName = selGl.nama;
                }
                if (this.assignedForemanId) {
                    const selFm = this.fmUsers.find(u => String(u.id) === String(this.assignedForemanId));
                    if (selFm) this.bundleFmName = selFm.nama;
                }
                ...
                for (let r = 0; r < 9; r++) {
                    ...
                    if (appVal && appVal.toUpperCase().includes('TYPE PALLET')) {
                        ...
                        this.cols.forEach(c => {
                            ...
                            if (v) results[c] = (v === 'ok' ? 'âœ“' : v === 'ng' ? 'âœ—' : v);
                        });
                    }
                ...
                if (this.bundleGlSig) payload.paraf_gl = this.bundleGlSig;
                if (this.bundleFmSig) payload.paraf_foreman = this.bundleFmSig;
            ...
            async handleSave() {
                ...
                if (this.isLeader && ['draft', 'revision'].includes(this.status)) {
                    ...
                    // Support 2 formats:
                    // a) d.nominal (angka > 0) + d.method  ÃƒÂ¢Ã¢â‚¬Â Ã‚Â  format baru via DimModal
                    // b) d.label (teks, mis: "Ø15MM +0.5/-0.5") + d.method  ÃƒÂ¢Ã¢â‚¬Â Ã‚Â  format lama
                    let filledCount = 0;
                    for (let i = 0; i < 7; i++) {
                ...
                if (this.isForeman && (this.status === 'waiting_foreman' || this.status === 'waiting_qc_approval')) {
                    if (!this.bundleFmSig) {
                ...
                if (this.isOperator && this.isQCSectionOpen) {
                    ...
                    // Validasi Alasan Overtime
                    if (this.samplingProgressData?.isOverdue && !(this.catatan || '').trim()) {
                        this.showToast('error', 'Waktu Inspeksi Overtime! Wajib mengisi alasan di kotak Catatan/Remarks.');
                        setTimeout(() => {
                    ...
                    // Cek TTD Operator QC -  wajib bundlePrepSig (TTD milik Operator sendiri)
                    ...
                    // Validasi Kecepatan Inspeksi (Anti-Rushing / Anti-Nembak)
                    if (this.waktuMulai) {
                        const hasDimensi = this.dimStd && this.dimStd.some(d => d.item && d.item.trim() !== '');
                        const secondsPerSample = hasDimensi ? (parseFloat(this.ctDimensi) || 0) : (parseFloat(this.ctTanpaDimensi) || 0);
                        const sampleColsCount = this.cols.filter(c => typeof c === 'number').length;
                        
                        if (secondsPerSample > 0 && sampleColsCount > 0) {
                            const totalTargetSecs = secondsPerSample * sampleColsCount;
                            const start = new Date(this.waktuMulai).getTime();
                            const diffSecs = Math.max(0, Math.floor((Date.now() - start) / 1000));
                            
                            if (diffSecs < totalTargetSecs * 0.5) {
                                window.Swal.fire({
                                    title: 'TERLALU CEPAT! ⚡',
                                    html: 'Waktu penyelesaian Anda jauh di bawah standar target. Pastikan tidak ada pengecekan fisik yang dilewati.<br><br><b>Jika memang sudah selesai dengan valid, mohon sebutkan alasannya:</b>',
                                    icon: 'warning',
                                    input: 'textarea',
                                    inputPlaceholder: 'Ketik alasan Anda di sini (Wajib)...',
                                    ...
                                    preConfirm: (reason) => {
                                        if (!reason || reason.trim() === '') {
                                            window.Swal.showValidationMessage('Alasan wajib diisi!');
                                        }
                                        return reason;
                                    }
                                }).then((result) => {
                                    if (result.isConfirmed) {
                                        // Tambahkan catatan anomali secara otomatis
                                        const anomalyNote = `[ANOMALI WAKTU TERLALU CEPAT] Alasan Operator: ${result.value.trim()}`;
                                ...
                                return; // Hentikan eksekusi sinkronus, tunggu input dari pop-up
                            }
                        }
                    }
                ...
                if (this.isForeman && (this.status === 'waiting_qc_approval' || this.status === 'waiting_foreman')) {
                    if (!this.bundleFmSig) {
            ...
            async executeSave() {
                this.saving = true;
                try {
                    // Khusus QC Foreman & GL verifikasi bottom — gunakan /sign endpoint
                    if (this.isGroupLeader && this.status === 'waiting_qc_approval') {
                        await apiFetch(`/api/item-check/${this.editId}/sign`, {
                            method: 'POST',
                            body: JSON.stringify({ role: 'gl_bottom', signature: this.bundleGlSig })
                        });
                        localStorage.removeItem(`li_form_draft_${this.editId || 'new'}`);
                        this.showToast('success', 'Verifikasi GL selesai! Dokumen diteruskan ke Foreman.');
                        setTimeout(() => window.location.href = APP_URL + '/item-check', 1000);
                        return;
                    }

                    if (this.isForeman && (this.status === 'waiting_qc_approval' || this.status === 'waiting_foreman')) {
                        const signRes = await apiFetch(`/api/item-check/${this.editId}/sign`, {
                            method: 'POST',
                    ...
                    // Update the local status immediately to prevent background auto-save from reverting the DB!
                    this.status = target;
                    const payload = this.buildPayload(target);
                    let saveRes;
                    if (this.editId) saveRes = await apiFetch(`/api/item-check/${this.editId}`, { method: 'PUT', body: JSON.stringify(payload) });
                    else saveRes = await apiFetch('/api/item-check', { method: 'POST', body: JSON.stringify(payload) });

                    localStorage.removeItem(`li_form_draft_${this.editId || 'new'}`);

                    this.handleCreateQPR(saveRes, target);

                    if (!this.showQprPrompt) {
                        this.showToast('success', 'Berhasil disimpan!');
                        if (this.tandemUnfinishedId) {
                            this.showToast('info', 'Mengalihkan ke pasangan tandem yang belum selesai...');
                            setTimeout(() => window.location.href = APP_URL + '/item-check/' + this.tandemUnfinishedId + '/form', 1000);
                        } else {
                            setTimeout(() => window.location.href = APP_URL + '/item-check', 1000);
                        }
                    }
            ...
            // ——— QPR AUTO-TRIGGER INTEGRATION ———
            // Create the JavaScript logic to detect an "NG" input specifically in a "KEEPER" column 
            // and trigger the showQprPrompt workflow.
            handleCreateQPR(saveRes, target) {
                const savedData = saveRes?.data ?? saveRes;
                const autoQprId = savedData?.qpr_id ?? null;
                
                // Show prompt if QPR exists and we are at a terminal state (finished or waiting_qc_approval)
                const isTerminalSave = ['finished', 'waiting_qc_approval'].includes(target);
                if (autoQprId && isTerminalSave) {
                    this.pendingQprId = autoQprId;
                    this.qprId = autoQprId; // Prevent subsequent prompts
            ...
            async loadFromHistory() {
                ...
                try {
                    const res = await apiFetch(`/api/item-check/search?q=${this.searchQuery}`);
            ...
            async selectSearchResult(item) {
                ...
                if (incomingJob && incomingJob !== 'â€”' && incomingJob !== '-') {
            ...
            async submitAllRevisions() {
                ...
                const isGL = this.isGroupLeader && this.status === 'waiting_qc_approval';
                this.confirmTitle = isGL ? 'Kirim Balik ke Operator' : 'Kirim Balik ke Leader';
                this.confirmMessage = isGL 
                    ? 'Apakah Anda yakin ingin mengirim catatan revisi ini ke Operator? Dokumen akan dikembalikan ke status REVISI.'
                    : 'Apakah Anda yakin ingin mengirim semua catatan revisi ini kembali ke Leader?';
            ...
            async executeSubmitRevisions() {
                ...
                try {
                    ...
                    for (const [key, note] of Object.entries(this.pendingFieldRevisions)) {
                        formattedRevisions[key] = { catatan: note || '-', resolved: false };
                    }

                    const isGL = this.isGroupLeader && this.status === 'waiting_qc_approval';
                    const payload = { 
                        field_revisions: formattedRevisions,
                        catatan_revisi: isGL ? 'Revisi dari GL: ' + Object.keys(formattedRevisions).join(', ') : undefined
                    };
                    
                    await apiFetch(`/api/item-check/${this.editId}/field-revisions`, { 
                        method: 'POST', 
                        body: JSON.stringify(payload) 
                    });

                    const msg = isGL ? 'Catatan revisi berhasil dikirim ke Operator' : 'Semua catatan revisi berhasil dikirim ke Leader';
            ...
            async executeResolveRevision() {
                ...
                try {
                    const res = await apiFetch(`/api/item-check/${this.editId}/resolve-revision`, {
                        method: 'POST',
                        body: JSON.stringify({
                            field: key,
                            resolved: true,
            ...
            async updateAssignment() {
                ...
                try {
                    await apiFetch(`/api/item-check/${this.editId}/assign`, {
                        method: 'POST',
            ...
            async startTask() {
                ...
                if (secondsPerSample <= 0) {
                    window.Swal.fire({
                        ...
                        html: '<b>QA Leader (Totok) belum mengatur Target Waktu (CT)</b> untuk Part ini.<br><br>Sistem tidak bisa mencatat performa tanpa target waktu. Silakan hubungi QA Leader untuk mengisi Master Item Check terlebih dahulu.',
                        icon: 'error',
                ...
                const minutesPerSample = (secondsPerSample / 60).toFixed(1).replace('.0', '');
                
                const confirmationText = `
                    <div class="text-left text-sm mt-3 space-y-3 text-slate-600">
                        <div class="p-3 bg-blue-50 border border-blue-100 rounded-lg">
                            <p class="font-black text-blue-800 mb-1 text-[11px] uppercase tracking-wider">⏱️ TARGET WAKTU INSPEKSI:</p>
                            <ul class="list-disc pl-4 space-y-1 mt-2 text-xs">
                                <li>Total Waktu Maksimal: <b class="text-slate-800">${this.waktuStandarText}</b></li>
                                <li>Target Waktu per Sampling: <b class="text-slate-800">${minutesPerSample} Menit</b> (${secondsPerSample} detik)</li>
                            </ul>
                        </div>
                        <p class="text-rose-600 font-bold text-xs bg-rose-50 p-2.5 rounded-lg border border-rose-100 text-center">
                            Waktu akan langsung berjalan setelah menekan tombol di bawah. Harap segera lakukan inspeksi!
                        </p>
                    </div>
                `;
                ...
                try {
                    const res = await apiFetch(`/api/item-check/${this.editId}/start`, { method: 'POST' });
                    this.waktuMulai = res.waktu_mulai;
                    this.showToast('success', 'Waktu pengukuran dimulai');
                } catch (e) { this.showToast('error', e.message); }
            },
            ...
            promptItemCheckRevision() {
                ...
                apiFetch(`/api/item-check/${this.editId}`, { method: 'PUT', body: JSON.stringify(payload) })
                    .then(() => {
            ...
            appendLeaderRevisionRecord(label, fromVal, toVal) {
                ...
                const text = `[Leader QA] Koreksi ${label}: ${fromStr} â†’ ${toStr}`;
                const dup = this.revRecords.some((r) => (r.record || '').trim() === text);
                if (dup) return;
                this.revRecords.push({
                    date: new Date().toISOString().slice(0, 10),
            ...
            clearSketchWithRevision() {
                if (this.shouldLogLeaderRevision && this._leaderEditBaseline?.sketch === 'ada') {
                    this.appendLeaderRevisionRecord('Sketsa', 'ada', '- ');
                    this._leaderEditBaseline.sketch = '- ';
                }
                this.sketchUrl = null;
            },
            ...
            async handleSaveDraft(isSilent = false) {
                ...
                try {
                    ...
                    if (isEditing) {
                        ...
                        await apiFetch(`/api/item-check/${this.editId}`, { method: 'PUT', body: JSON.stringify(payload) });
                    } else {
                        // Buat record baru sebagai draft di server
                        const res = await apiFetch('/api/item-check', { method: 'POST', body: JSON.stringify(payload) });
                        if (res.id) {
                            this.editId = res.id;
            ...
            async flipTandem(url) {
                if (this.saving) return;
                
                // Jika dokumen sudah selesai/terkunci, langsung pindah tanpa menyimpan
                if (['locked', 'finished', 'approved'].includes(this.status)) {
                    this.showToast('success', 'Membalik part...');
                    setTimeout(() => window.location.href = url, 300);
                    return;
                }

                this.saving = true;
                this.showToast('info', 'Menyimpan draft sebelum flip...');
                try {
                    const payload = this.buildPayload(this.status || 'draft');
                    await apiFetch(`/api/item-check/${this.editId}`, { method: 'PUT', body: JSON.stringify(payload) });
                    this.showToast('success', 'Draft tersimpan, membalik part...');
                    setTimeout(() => window.location.href = url, 500);
                } catch (e) {
                    this.showToast('error', 'Gagal menyimpan draft: ' + e.message);
                    this.saving = false;
                }
            },
            // ==========================================
            ...
            saveSketchAnnotation() {
                ...
                setTimeout(() => {
                    try {
                        // Build SVG string directly from state - more reliable than XMLSerializer
            ...
            svgRenderAll() {
                ...
                for (const s of this.svgShapes) {
                    if (s.type === 'rect') {
                        ...
                        const maxW = s.w || 20;
                        // Invisible drag background rect - makes clicking/dragging easy
                    ...
                    if (this.svgSelected === s.id) {
                        ...
                        if (s.type === 'line' || s.type === 'arrow') {
                            ...
                            // Segment midpoint handles (click to add waypoint) - shown as small cyan circles
                            for (let si = 0; si < allPts.length - 1; si++) {
                                const mx = (allPts[si].x + allPts[si+1].x) / 2;
                                const my = (allPts[si].y + allPts[si+1].y) / 2;
                            ...
                            // Waypoint handles (draggable) - shown as medium blue circles
                            const wps = s.waypoints || [];
                            for (const wp of wps) {
