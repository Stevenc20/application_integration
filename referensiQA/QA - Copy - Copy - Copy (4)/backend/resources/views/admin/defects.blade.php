...
<div class="um-wrap max-w-[1100px] mx-auto p-7 pb-[70px]" x-data="defectManagement({ apiUrl: '{{ url('/api/defects') }}' })">
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-3 mb-6">
        <div>
            ...
            <h1 class="text-3xl font-black text-slate-800 tracking-tight mb-1">Master Jenis Defect / NG</h1>
            <p class="text-sm text-slate-500 font-medium">Kelola daftar jenis kerusakan barang / Not Good</p>
        </div>
    ...
    <div class="flex gap-3 mb-4 bg-white border-2 border-slate-100 rounded-2xl p-3 shadow-sm">
        <div class="relative flex-1">
            ...
            <input x-model="search" class="um-input w-full pl-10 pr-4 py-2.5 bg-white border-2 border-slate-200 rounded-xl text-sm font-bold text-slate-700 outline-none" placeholder="Cari kode, nama, atau kategori defect…">
        </div>
            ...
            <table class="w-full text-left min-w-[800px]">
                <thead>
                    <tr class="bg-slate-50 border-b-2 border-slate-100">
                        @foreach(['Kode Defect', 'Nama Defect (NG)', 'Kategori', 'Status', ''] as $h)
                ...
                <tbody>
                    <template x-if="loading">
                        <template x-for="i in 3">
                            <tr class="border-b border-slate-50">
                                <td class="p-5" colspan="5"><div class="um-shimmer h-6 w-full"></div></td>
                            </tr>
                        </template>
                    </template>
                    <template x-if="!loading && filteredItems.length === 0">
                        <tr>
                            <td colspan="5" class="p-20 text-center">
                                ...
                                <p class="text-base font-black text-slate-800 mb-1">Tidak ada data defect ditemukan</p>
                            </td>
                        </tr>
                    </template>
                    <template x-for="item in filteredItems" :key="item.id">
                        <tr class="um-row border-b border-slate-50 last:border-b-0">
                            <td class="px-5 py-4">
                                <span class="px-3 py-1 bg-red-50 border border-red-200 text-red-600 rounded-lg text-[11px] font-black font-mono" x-text="item.code"></span>
                            </td>
                            <td class="px-5 py-4">
                                <span class="text-sm font-black text-slate-800" x-text="item.name"></span>
                            </td>
                            <td class="px-5 py-4">
                                <span class="px-2 py-1 bg-slate-100 text-slate-700 rounded text-[10px] font-black uppercase tracking-wider" x-text="item.category"></span>
                            </td>
                            <td class="px-5 py-4">
            ...
            <div class="p-6 bg-slate-50 border-b-2 border-slate-100 flex items-center justify-between">
                <div class="flex items-center gap-4">
                    <div class="w-12 h-12 bg-red-50 border-2 border-red-100 rounded-2xl flex items-center justify-center text-2xl">
                        <span x-text="selectedItem ? '✏️' : '⚠️'"></span>
                    </div>
                    <div>
                        <h3 class="text-lg font-black text-slate-800 leading-tight" x-text="selectedItem ? 'Edit Defect' : 'Tambah Defect Baru'"></h3>
                        <p class="text-xs text-slate-400 font-bold uppercase tracking-wider" x-text="selectedItem ? 'Ubah data defect' : 'Isi form untuk defect baru'"></p>
                    </div>
                </div>
            ...
            <form @submit.prevent="handleSubmit()" class="p-6 space-y-4">
                <div class="grid grid-cols-2 gap-4">
                    <div class="space-y-1.5">
                        <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Kode Defect *</label>
                        <input x-model="form.code" required class="w-full px-4 py-2.5 bg-slate-50 border-2 border-slate-200 rounded-xl text-sm font-bold text-slate-700 outline-none focus:border-red-600 transition-all" placeholder="Misal: DENT">
                    </div>
                    <div class="space-y-1.5">
                        <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Kategori *</label>
                        <input x-model="form.category" required class="w-full px-4 py-2.5 bg-slate-50 border-2 border-slate-200 rounded-xl text-sm font-bold text-slate-700 outline-none focus:border-red-600 transition-all" placeholder="Misal: Visual, Dimensi">
                    </div>
                </div>

                <div class="space-y-1.5">
                    <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Nama Defect (NG) *</label>
                    <input x-model="form.name" required class="w-full px-4 py-2.5 bg-slate-50 border-2 border-slate-200 rounded-xl text-sm font-bold text-slate-700 outline-none focus:border-red-600 transition-all" placeholder="Misal: Penyok (Dent) di permukaan">
                </div>

                <div x-show="selectedItem" class="pt-2">
                    <label class="flex items-center gap-2 cursor-pointer">
                        ...
                        <span class="text-sm font-bold text-slate-700">Aktif (Muncul di Pilihan)</span>
                    </label>
                </div>
                ...
                <div class="pt-4 border-t-2 border-slate-50 flex gap-3">
                    ...
                    <button type="submit" :disabled="submitting" class="flex-[2] py-3 bg-red-600 text-white rounded-2xl text-sm font-black shadow-lg shadow-red-600/20 disabled:opacity-50 flex items-center justify-center gap-2">
                        ...
                        <span x-text="selectedItem ? 'Simpan Perubahan' : 'Tambah Defect'"></span>
                    </button>
                </div>
            </form>
        </div>
...
document.addEventListener('alpine:init', () => {
    Alpine.data('defectManagement', (config) => ({
        items: [],
        search: '',
        loading: false,
        submitting: false,
        ...
        showModal: false,
        selectedItem: null,
        form: { code: '', name: '', category: 'Visual', is_active: true },
        ...
        openModal(item = null) {
            ...
            if (item) {
                ...
                this.form = { code: '', name: '', category: 'Visual', is_active: true };
            }
            this.showModal = true;
        },
        ...
        async handleSubmit() {
            ...
            try {
                if (this.selectedItem) {
                    ...
                    this.showToast('Data defect diperbarui!');
                } else {
                    await window.axios.post(config.apiUrl, this.form);
                    this.showToast('Defect baru ditambahkan!');
                }
        ...
        async deleteItem(id) {
            ...
            try {
                ...
                this.showToast('Defect berhasil dihapus!');
                this.fetchItems();
            } catch (err) {
                this.showToast('Gagal menghapus defect', 'error');
            }
        }
    }));
});
